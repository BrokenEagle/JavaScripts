...
/********
 *  The rate limit for reads is a hard 10 per second. This may need to be reduced per userscript to avoid 429 errors.
...
Danbooru.query = async function (path, url_addons = {}, {default_val = null, long_format = false, domain = '', notify = false} = {}) {
    const printer = Debug.getFunctionPrint('Danbooru.query');
    if (Danbooru.down) {
        printer.log('Downbooru... returning default value.')
        if (notify) {
            Notice.notice("Danbooru is down.");
        }
        return default_val;
    }
    ...
    _updateReadRequests(+1);
    printer.logLevel({path, url_addons, default_val, long_format, domain, notify}, Debug.ALL);
    let record_key = Debug.recordKey('Danbooru.query', () => `${path};${Utility.renderParams(url_addons)}`);
    ...
    return results.then(
        ...
        (resp) => {
            let error = Network.processError(resp, "Danbooru.query");
            ...
            if (resp.getResponseHeader('content-type').indexOf('text/html') >= 0) {
                Danbooru.down = true;
                if (Danbooru.error_domname) {
                    let status = (resp.responseText.indexOf("<title>Downbooru</title>") > 0 ? 'downbooru' : 'unknown')
                    jQueryProxy(Danbooru.error_domname).text(status);
                }
            }
            return default_val;
        }
...
Danbooru.queryPageItems = async function (type, limit, {url_addons = {}, batches = null, reverse = false, long_format = false, page = null, domain = "", domname = null, notify = false} = {}) {
    const printer = Debug.getFunctionPrint('Danbooru.queryPageItems');
    if (Danbooru.down) {
        printer.log('Downbooru... returning empty array.')
        if (notify) {
            Notice.notice("Danbooru is down.");
        }
        return [];
    ...
    let page_addon = (Utility.isInteger(page) ? {page: `${page_modifier}${page}`} : {});
    let limit_addon = {limit};
    let batch_num = 1;
    var return_items = [];
    await _initializePageCounter(type, limit, url_addons, reverse, long_format, page, domain, domname, notify);
    while (true) {
        let request_addons = Utility.mergeObjects(url_addons, page_addon, limit_addon);
        let temp_items = await Danbooru.query(type, request_addons, {default_val: [], long_format, domain, notify});
        return_items = Utility.concat(return_items, temp_items);
        let lastid = Danbooru.getNextPageID(temp_items, reverse);
        _updatePageCounter(domname, limit, lastid);
...
Danbooru.queryIDItems = async function (type, id_list, limit, {id_addon = null, other_addons = {}, long_format = false, domain = "", domname = null, notify = false} = {}) {
    const printer = Debug.getFunctionPrint('Danbooru.queryIDItems');
    if (Danbooru.down) {
        printer.log('Downbooru... returning empty array.')
        if (notify) {
            Notice.notice("Danbooru is down.");
    ...
    if (!Utility.isIDList(id_list)) {
        throw new Error("Danbooru.queryIDItems: Invalid ID list");
    }
    ...
    _setIDCounter(domname, total_pages);
    var end;
    for (let i = 0; i < id_list.length; i = end) {
        end = i + limit;
        let sublist = id_list.slice(i, end);
        let request_addons = Utility.mergeObjects(id_addon(sublist), other_addons, limit_addon);
        printer.logLevel("Querying", type, `${i + 1} - ${end} / ${id_list.length}`, sublist, Debug.INFO);
        let temp_items = await Danbooru.query(type, request_addons, {default_val: [], long_format, domain, notify});
        return_items = Utility.concat(return_items, temp_items);
        _setIDCounter(domname, --total_pages);
    }
...
Danbooru.updateSetup = async function () {
    ...
    const waitCondition = () => (NUM_WRITE_REQUESTS >= Danbooru.max_write_requests);
    PENDING_UPDATES.count += 1;
    _showPendingUpdateNotice();
    if (waitCondition()) {
...
Danbooru.initializeHighlights = function () {
    if (!POST_HIGHLIGHT_ENABLED) {
        Utility.setCSSStyle(Utility.sprintf(HIGHLIGHT_CSS, JSPLib.shortcut), 'Danbooru.highlights');
...
function _checkAPIRateLimit(resp) {
...
async function _initializePageCounter(type, limit, url_addons, reverse, long_format, page, domain, domname, notify) {
    const printer = Debug.getFunctionPrint('Danbooru.initializePageCounter');
    if (domname && Utility.isInteger(page)) {
        let latest_id = jQueryProxy(domname).data('latest-id');
        if (!Utility.isInteger(latest_id)) {
            let request_addons = Utility.mergeObjects(url_addons, {limit: 1}, {only: 'id'});
            if (!reverse) {
                request_addons.page = 'a0';
            }
            let latest_item = await Danbooru.query(type, request_addons, {default_val: [], long_format, domain, notify});
...
function _updatePageCounter(domname, limit, page) {
    const printer = Debug.getFunctionPrint('Danbooru.updatePageCounter');
    if (domname) {
        let latest_id = jQueryProxy(domname).data('latest-id');
        if (Utility.isInteger(latest_id)) {
            let current_counter = (Utility.isInteger(page) ? Math.abs(Math.ceil((latest_id - page) / limit)) : 0);
            jQueryProxy(domname).text(current_counter);
            printer.logLevel(current_counter, latest_id, page, Debug.INFO);
        }
    }
...
function _setIDCounter(domname, counter) {
    if (domname) {
...
function _highlightPost(post_id, highlight_on) {
    if (!POST_HIGHLIGHT_ENABLED) return;
    if (!(post_id in POST_HIGHLIGHT_DICT)) {
        if (highlight_on) {
            let $post = jQueryProxy('#post_' + post_id);
            let $highlight = POST_HIGHLIGHT_DICT[post_id] = jQueryProxy(`<div class="${JSPLib.shortcut}-post-highlight"></div>`);
...
function _updateReadRequests(change) {
...
function _updatePostVotes (params, post_data, $post_votes) {
    ...
    if (/(?:^| )(?:upvote:self|downvote:self|-?fav:me)(?: |$)/.test(params.post.tag_string)) {
        Danbooru.query('post_votes', {search: {post_id: post_data.id, user_id: DanbooruProxy.CurrentUser.data('id')}, limit: 1})
            .then((vote_data) => {
...
JSPLib.initializeModuleProperty('Danbooru', 'down', false, (set_value, stored_value, original_value) => {
    switch (stored_value) {
        case 'true':
            return true;
        case 'false':
...
JSPLib.initializeModule('Danbooru', ['categories']);
