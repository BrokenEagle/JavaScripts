...
Object.defineProperties(JSPLib, {
    program_shortcut: {
        ...
        set(shortcut) {
            ...
            JSPLib.program_prefix = shortcut_upper + ':';
            JSPLib.program_timer = shortcut_upper + '-';
            JSPLib.program_regex = RegExp(`^${shortcut}-(.*)`);
            PROGRAM_SHORTCUT = shortcut;
        },
        enumerable: true,
    },
    program_name: {
        get() {
            return JSPLib._program_name;
        },
        set(name) {
            JSPLib.program_id = name.replace(/([a-z])([A-Z])/g, '$1-$2').replace(/[\s_]+/g, '-').toLowerCase();
            JSPLib._program_name = name;
        },
        enumerable: true,
    },
    _jquery_installed: {
        get() {
            JQUERY_INSTALLED ||= (typeof jQuery === 'function');
    ...
    _danbooru_installed: {
        get() {
            DANBOORU_INSTALLED ||= (typeof Danbooru === 'object');
...
JSPLib.initializeModule(null, {
    nonwritable: ['_active_script', '_sandboxed', '_page_context', '_window', '_document', '_gm_info', '_window_jsp', 'UID', 'info', 'program']
});
