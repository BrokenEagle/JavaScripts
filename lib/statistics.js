...
JSPLib.statistics.outputAdjustedMean = function (tablename = '') {
    const printer = JSPLib.debug.getFunctionPrint('statistics.outputAdjustedMean');
    if (Object.keys(JSPLib.debug._records).length === 0) {
        return;
    }
    if (tablename !== '') {
        printer.debuglogLevel(`====${tablename} Metrics====`, JSPLib.debug.INFO);
    }
    let outputtime = {};
    ...
    for (let i = 0;i < outputkeys.length;i++) {
        ...
        printer.debuglogLevel(type + ':', "num", values.length, "avg", parseFloat(adjaverage.toFixed(2)), "rem", values.length - adjvalues.length, JSPLib.debug.INFO);
    }
};

JSPLib.statistics.addPageStatistics = function () {
    JSPLib.debug.debugExecute(() => {
        JSPLib._window.addEventListener('beforeunload', () => {
            this.outputAdjustedMean(JSPLib.program_name);
        });
    });
};
