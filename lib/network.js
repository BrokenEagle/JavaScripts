...
JSPLib.network.getNotify = function (url, {url_addons = {}, custom_error = "", ajax_options, xhr_options} = {}) {
    let full_url = url + (Object.keys(url_addons).length ? '?' + JSPLib.utility.renderParams(url_addons) : '');
    ...
    return this.get(full_url, {ajax_options, xhr_options}).then(
        ...
        (error) => {
            ...
            let error_key = `${url}?${JSPLib.utility.renderParams(url_addons)}`;
...
JSPLib.network.ajax = function (url, options) {
    const printer = JSPLib.debug.getFunctionPrint('network.ajax');
    if (!JSPLib._jquery_installed) {
        return Promise.resolve(null);
    }
    printer.debuglogLevel({url, options}, JSPLib.debug.ALL);
...
JSPLib.network.rateLimit = async function (module) {
    const printer = JSPLib.debug.getFunctionPrint('network.rateLimit');
    while (JSPLib[module].num_network_requests >= JSPLib[module].max_network_requests) {
        printer.debuglogLevel("Max simultaneous network requests exceeded! Sleeping...", JSPLib.debug.WARNING);
...
JSPLib.network.processError = function (error, funcname) {
    const printer = JSPLib.debug.getFunctionPrint('network.processError');
    var ret = null;
    ...
    if (error.status > 0) {
        if (message.match(/<\/html>/i)) {
            ...
            try {
                ...
                if (JSPLib.utility.isHash(parse_message)) {
        ...
        notice_html = JSPLib.utility.sprintf(this._notify_header, `HTTP ${error.status}:`);
    } else {
        notice_html = JSPLib.utility.sprintf(this._notify_header, "Network error:");
    }
    notice_html += ' ' + message;
...
JSPLib.network._notify_header = '<span style="font-size:16px;line-height:24px;font-weight:bold;font-family:sans-serif">%s</span>';
