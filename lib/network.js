...
JSPLib.network.GM_XHR = function () {
    const open_params = ['type','url','async','username','password','responseType'];
    ...
    this.onresponse = function (handler) {
        ...
        return function (resp) {
            ['readyState','responseHeaders','responseText','response','status','statusText'].forEach(function (key) {
                xhr[key] = resp[key];
            });
            if (xhr[handler]) {
                xhr[handler](xhr);
    ...
    this.send = function(data) {
        const gm_params = ['url','headers','data','responseType','cookie','binary','nocache','revalidate','timeout','context','overrideMimeType','anonymous','fetch','user','password'];
...
JSPLib.network.getNotify = async function (url,url_addons={},custom_error,xhr_options={}) {
    let full_url = url + (Object.keys(url_addons).length ? '?' + JSPLib._jQuery.param(url_addons) : '');
    JSPLib.debug.recordTime(full_url, 'Network');
    try {
        return await this.ajax({url: full_url, xhrFields: xhr_options});
    } catch(error) {
        //Swallow exception... will return false value
        error = this.processError(error,"network.getNotify");
        let error_key = `${url}?${JSPLib._jQuery.param(url_addons)}`;
        this.logError(error_key,error);
        this.notifyError(error,custom_error);
        return false;
    } finally {
...
JSPLib.network.getData = function (data_url,xhr_options={}) {
    JSPLib.debug.recordTime(data_url, 'Network');
    return this.ajax({
            url: data_url,
            dataType: 'binary',
            xhrFields: Object.assign({responseType: 'blob'}, xhr_options),
    }).then((data, _ , resp)=>{
...
JSPLib.network.getDataSize = function (image_url,xhr_options={}) {
...
//These support xhrOptions, and are copied from jQuery 3.6.0

JSPLib.network.ajax = function (url, options) {
...
['get','post'].forEach((method)=>{
    JSPLib.network[method] = function(url,data,callback,type,ajax_options={},xhr_options={}) {
        ...
        if (JSPLib.utility._isHash(url)) {
            ajax_options = JSPLib.utility.joinArgs(url, ajax_options);
...
JSPLib.network.getJSON = function(url,data,callback,ajax_options,xhr_options) {
    return this.get(url,data,callback,'json',ajax_options,xhr_options);
};

JSPLib.network.getScript = function (url,callback,ajax_options,xhr_options) {
    ajax_options = JSPLib.utility.joinArgs({cache: true}, ajax_options);
    return this.get(url, {}, callback, 'script', ajax_options, xhr_options);
};
...
JSPLib.network.incrementCounter = function (module) {
    ...
    this.counter_domname && JSPLib._jQuery(this.counter_domname).html(JSPLib[module].num_network_requests);
};
