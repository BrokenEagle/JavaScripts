...
JSPLib.network.getNotify = async function (url,url_addons={},custom_error) {
    try {
        ...
        JSPLib.network.logError(error,error_key);
...
JSPLib.network.rateLimit = async function (module) {
    while (JSPLib[module].num_network_requests >= JSPLib[module].max_network_requests) {
        JSPLib.debug.debuglogLevel("network.rateLimit - Max simultaneous network requests exceeded! Sleeping...",JSPLib.debug.WARNING);
