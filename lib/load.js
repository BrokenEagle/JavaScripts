...
Load.programInitialize = function (entry_func, {function_name = null, required_variables = [], required_selectors = [], optional_selectors = [], max_retries = DEFAULT_MAX_RETRIES, timer_interval = DEFAULT_TIMER_INTERVAL} = {}) {
    ...
    if (JSPLib.name && function_name === null) {
        if (JSPLib._window_jsp.program[JSPLib.name]) return;
...
Load.preloadScript = function ({default_data = {}, reset_data = {}, initialize_func = null, broadcast_func = null, program_css = null, light_css = null, dark_css = null, danbooru_userscript = true} = {}) {
...
function _isScriptEnabled() {
    ...
    return domains.split(',').includes(CURRENT_SUBDOMAIN);
}
