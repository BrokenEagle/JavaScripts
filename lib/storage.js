...
JSPLib.storage.checkLocalDB = async function (key,validator,max_expires,database=JSPLib.storage.danboorustorage) {
    if (this.use_storage) {
        var cached = await this.retrieveData(key,database);
        if (cached === null) {
    ...
    if (this.use_storage) {
        ...
        await database.iterate((value,key)=>{
            if (key.match(regex)) {
                ...
                if (pruned_keys.length >= this.prune_limit) {
...
JSPLib.storage._configuration = {
    nonenumerable: ['_active_script','_window','_channel','_adjustCounter','_addItemCacheInfo','_configuration'],
