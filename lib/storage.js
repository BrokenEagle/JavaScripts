...
storage.getStorageData = function (key, storage, {default_val = null, bypass = false} = {}) {
    let storage_type = _getStorageType(storage);
    var return_val;
...
storage.setStorageData = function (key, data, storage) {
...
storage.removeStorageData = function (key, storage) {
    storage.invalidateStorageData(key, storage);
    storage.removeItem(key);
};

storage.invalidateStorageData = function (key, storage) {
    let storage_type = _getStorageType(storage);
...
storage.checkStorageData = function (key, storage, {validator = storage.localSessionValidator, default_val = null, bypass = false} = {}) {
    ...
    let data = storage.getStorageData(key, storage, {bypass});
...
storage.inMemoryStorage = function (key, storage) {
    let storage_type = _getStorageType(storage);
