...
Storage.indexed_db_available = Boolean(Storage.danboorustorage) && Storage.danboorustorage.supports(Storage.danboorustorage.INDEXEDDB);
...
Storage.getStorageData = function (key, store, {default_val = null, bypass = false} = {}) {
    ...
    if (!bypass && key in MEMORY_STORAGE[storage_type]) {
        ...
        let record_key = Debug.recordKey('Storage.getStorageData', key);
        Debug.recordTime('Storage', record_key);
        let data = store.getItem(key);
        Debug.recordTimeEnd('Storage', record_key);
        try {
...
Storage.retrieveData = async function (key, {bypass_cache = false, timeout = null, default_val = null, database = Storage.danboorustorage} = {}) {
    const printer = Debug.getFunctionPrint('Storage.retrieveData');
    if (!Storage.indexed_db_available) return default_val;
    if (!bypass_cache) {
        let data = Storage.getIndexedSessionData(key, {default_val, database});
        if (data) {
            printer.logLevel("Found item (Session):", key, Debug.VERBOSE);
            return data;
        }
    }
    let record_key = Debug.recordKey('Storage.retrieveData', key);
    Debug.recordTime('IndexDB', record_key);
    let data = await _databaseWait(database, 'getItem', key, timeout, default_val);
    Debug.recordTimeEnd('IndexDB', record_key);
    if (data !== null) {
        printer.logLevel(`Found item (IndexDB):`, key, Debug.VERBOSE);
...
Storage.saveData = function (key, value, {database = Storage.danboorustorage} = {}) {
    if (!Storage.indexed_db_available) return Promise.resolve();
...
Storage.batchRetrieveData = async function (keylist, {timeout = null, database = Storage.danboorustorage} = {}) {
    var found_session, found_database;
    const printer = Debug.getFunctionPrint('Storage.batchRetrieveData');
    if (!Storage.indexed_db_available || typeof database.getItems !== 'function') return {};
    ...
    let record_key = Debug.recordKey('Storage.batchRetrieveData', keylist);
    Debug.recordTime('IndexDB', record_key);
    let database_items = await _databaseWait(database, 'getItems', missing_keys, timeout, {});
    Debug.recordTimeEnd('IndexDB', record_key);
    Debug.execute(() => {
        found_database = Object.keys(database_items);
        if (found_database.length) {
            printer.log(`Found ${found_database.length} items (IndexDB):`, found_database);
        }
    }, Debug.VERBOSE);
    Debug.execute(() => {
...
Storage.batchSaveData = function (data_items, {database = Storage.danboorustorage} = {}) {
    if (!Storage.indexed_db_available || typeof database.setItems !== 'function') return Promise.resolve();
...
Storage.batchRemoveData = function (keylist, {database = Storage.danboorustorage} = {}) {
    if (!Storage.indexed_db_available || typeof database.removeItems !== 'function') return Promise.resolve();
    keylist.forEach((key) => {
        Storage.removeIndexedSessionData(key, {database});
    });
...
Storage.checkData = async function (key, {validator = Storage.indexedDBValidator, bypass_cache = false, timeout = null, default_val = null, max_expires = null, database = Storage.danboorustorage} = {}) {
    const printer = Debug.getFunctionPrint('Storage.checkData');
    if (!Storage.indexed_db_available) return default_val;
    var cached = await Storage.retrieveData(key, {bypass_cache, timeout, database});
    if (cached === null) {
...
Storage.batchCheckData = async function (keylist, {validator = Storage.indexedDBValidator, expiration = null, timeout = null, database = Storage.danboorustorage} = {}) {
    const printer = Debug.getFunctionPrint('Storage.batchCheckData');
    var cached = await Storage.batchRetrieveData(keylist, {timeout, database});
...
Storage.programCacheInfo = async function (data_regex, {database = Storage.danboorustorage} = {}) {
    ...
    var session_regex;
    var index_wait;
    if (data_regex instanceof RegExp) {
...
function _databaseWait(database, func_name, arg, timeout, default_val) {
    let p = Utility.createPromise();
    let resolved = false;
    if (Utility.isNumber(timeout) && timeout > 0) {
        Utility.sleep(timeout).then(() => {
            if (!resolved) {
                p.resolve(default_val);
                resolved = true;
            }
        });
    }
    database[func_name](arg).then((data) => {
        if (!resolved) {
            p.resolve(data);
            resolved = true;
        }
...
JSPLib.writeOnceReadMany(Storage, 'localSessionValidator', () => true, 'function');
JSPLib.writeOnceReadMany(Storage, 'indexedDBValidator', () => true, 'function');
