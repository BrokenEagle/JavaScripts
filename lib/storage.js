...
JSPLib.storage.retrieveData = async function (key,database=JSPLib.storage.danboorustorage) {
    if (!(JSPLib.storage.use_storage)) {
        return null;
    }
    let database_type = JSPLib.storage.use_indexed_db ? "IndexDB" : "LocalStorage";
    ...
    if (data !== null) {
        JSPLib.debug.debuglogLevel(`Found item (${database_type}):`,key,JSPLib.debug.VERBOSE);
...
JSPLib.storage.removeData = function (key,database=JSPLib.storage.danboorustorage) {
    if (JSPLib.storage.use_storage) {
...
JSPLib.storage.checkStorageData = function (key,validator,storage,default_val) {
    ...
    if (validator(key,data)) {
        JSPLib.debug.debuglogLevel("Data validated",key,JSPLib.debug.VERBOSE);
        return data;
    }
    JSPLib.debug.debuglogLevel("Data corrupted",key,JSPLib.debug.DEBUG);
    return default_val;
};
...
JSPLib.storage.checkLocalDB = async function (key,validator,max_expires,database=JSPLib.storage.danboorustorage) {
    if (JSPLib.storage.use_storage) {
        var cached = await JSPLib.storage.retrieveData(key,database);
        ...
        await database.iterate((value,key)=>{
            if (key.match(regex)) {
                if (JSPLib.storage.hasDataExpired(key,value)) {
                    ...
                    promise_array.push(JSPLib.storage.removeData(key,database));
                    pruned_items += 1;
                }
                total_items += 1;
...
JSPLib.storage.pruneEntries = function (modulename,regex,prune_expires,database=JSPLib.storage.danboorustorage) {
    ...
    if (!JSPLib.utility.validateExpires(expires,prune_expires)) {
        ...
        let promise_resp = JSPLib.storage.pruneStorage(regex,database).then(()=>{
        ...
        JSPLib.storage.setStorageData(expire_name, JSPLib.utility.getExpires(adjusted_prune_expires), localStorage);
        return promise_resp;
    } else {
...
JSPLib.storage.purgeCache = async function (regex,counter_domname,database=JSPLib.storage.danboorustorage) {
    ...
    await database.iterate((value,key)=>{
        if (key.match(regex)) {
            JSPLib.debug.debuglogLevel("Deleting",key,JSPLib.debug.DEBUG);
            let resp = JSPLib.storage.removeData(key,database).then(()=>{
    ...
    JSPLib.utility.notice(`Deleting ${purged_count} items...`);
...
JSPLib.storage.programCacheInfo = async function (program_shortcut,regex,database=JSPLib.storage.danboorustorage) {
    ...
    let index_wait = database.iterate((value,key)=>{
...
JSPLib.storage._configuration = {
    nonenumerable: ['_adjustCounter','_addItemCacheInfo','_configuration'],
    nonwritable: ['danboorustorage','use_indexed_db','use_local_storage','use_storage','_configuration']
};
