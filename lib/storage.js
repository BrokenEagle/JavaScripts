...
JSPLib.storage.pruneStorage = async function (regex,database=JSPLib.storage.danboorustorage) {
    ...
    if (database.removeItems) {
        ...
        for (let i = 0; i < delay_array.length; i += 50, current_batch++) {
            ...
            JSPLib.debug.debuglogLevel(`storage.pruneStorage - Pruning batch #${current_batch} of ${total_batches}`,JSPLib.debug.INFO);
...
JSPLib.storage._broadcastRX = function () {
    ...
    return function (event) {
        if (context._active_script && event.data.from !== context.UID) {
            JSPLib.debug.debuglogLevel('storage._broadcastRX:',`(${event.data.type}):`,event.data,JSPLib.debug.INFO);
