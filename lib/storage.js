...
JSPLib.storage.getStorageData = function (key, storage, {default_val = null, bypass_cache = false} = {}) {
    let storage_type = this._getStorageType(storage);
    if (bypass_cache || !(key in this.memory_storage[storage_type]) && key in storage) {
...
JSPLib.storage.getLocalData = function (key, {default_val = null, bypass_cache = false} = {}) {
    return this.getStorageData(key, localStorage, {default_val, bypass_cache});
};
...
JSPLib.storage.getSessionData = function (key, {default_val = null, bypass_cache = false} = {}) {
    return this.getStorageData(key, sessionStorage, {default_val, bypass_cache});
};
...
JSPLib.storage.getIndexedSessionData = function (key, {default_val = null, bypass_cache = false, database = JSPLib.storage.danboorustorage} = {}) {
    let session_key = this._getSessionKey(key, database);
    return this.getSessionData(session_key, {default_val, bypass_cache});
};
...
JSPLib.storage.batchRetrieveData = async function (self, keylist, {bypass_cache = false, database = JSPLib.storage.danboorustorage} = {}) {
...
JSPLib.storage.checkStorageData = function (self, key, storage, {validator = JSPLib.storage.localSessionValidator, bypass_cache = false, default_val = null} = {}) {
    let storage_type = this._getStorageType(storage);
    self.debug('logLevel', "Checking storage", key, JSPLib.debug.VERBOSE);
    if (!bypass_cache && (key in this.memory_storage[storage_type])) {
    ...
    let data = this.getStorageData(key, storage, {bypass_cache: true});
...
JSPLib.storage.checkLocalData = function (key, {validator = JSPLib.storage.localSessionValidator, bypass_cache = false, default_val = null} = {}) {
    return this.checkStorageData(key, localStorage, {validator, bypass_cache, default_val});
};

JSPLib.storage.checkSessionData = function (key, {validator = JSPLib.storage.localSessionValidator, bypass_cache = false, default_val = null} = {}) {
    return this.checkStorageData(key, sessionStorage, {validator, bypass_cache, default_val});
};

JSPLib.storage.checkLocalDB = async function (self, key, max_expires, {validator = JSPLib.storage.indexedDBValidator, bypass_cache = false, database = JSPLib.storage.danboorustorage} = {}) {
    if (this.use_storage) {
        var cached = await this.retrieveData(key, {bypass_cache, database});
        if (cached === null) {
...
JSPLib.storage.batchCheckLocalDB = async function (self, keylist, expiration, {validator = JSPLib.storage.indexedDBValidator, bypass_cache = false, database = JSPLib.storage.danboorustorage} = {}) {
    var cached = await this.batchRetrieveData(keylist, {bypass_cache, database});
...
JSPLib.storage.pruneCache = async function (self, regex, {database = JSPLib.storage.danboorustorage} = {}) {
    ...
    await database.iterate((value, key) => {
        if (key.match(regex)) {
            ...
            checked++;
            if (pruned_keys.length >= 100000) {
                self.debug('logLevel', "Maximum prune length reached!", JSPLib.debug.WARNING);
                return false;
            }
        }
    });
    self.debug('logLevel', `Pruning ${pruned_keys.length}/${checked} items!`, JSPLib.debug.INFO);
...
JSPLib.storage.pruneProgramCache = function (self, program_name, regex, prune_expires, {database = JSPLib.storage.danboorustorage} = {}) {
    let database_key = this._getDatabaseKey(database);
    let expire_name = `${program_name}-${database_key}-prune-expires`;
    ...
    JSPLib.debug.debugTime("pruneProgramCache");
    ...
    let promise_resp = this.pruneCache(regex, {database}).then(() => {
        ...
        JSPLib.debug.debugTimeEnd("pruneProgramCache");
...
JSPLib.storage.hasDataExpired = function (self, key, storeditem, max_expires) {
    if (typeof storeditem !== "object" || storeditem == null || (!('expires' in storeditem))) {
        self.debug('logLevel', key, "has no expires!", JSPLib.debug.DEBUG);
        return true;
    }
