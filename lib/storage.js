...
Storage.batchRetrieveData = async function (keylist, {database = Storage.danboorustorage} = {}) {
    ...
    printer.logLevel(() => {
        ...
        if (found_session.length) {
            return ["Found", found_session.length, "items (Session):", found_session];
        }
    }, Debug.VERBOSE);
    ...
    printer.logLevel(() => {
        ...
        if (found_database.length) {
            return [`Found ${found_database.length} items (${database_type}):`, found_database];
        }
    }, Debug.VERBOSE);
    printer.logLevel(() => {
        ...
        if (missing_list.length) {
            return ["Missing", missing_list.length, "items:", missing_list];
        }
    }, Debug.VERBOSE);
...
function _broadcastRX() {
    ...
    return function (event) {
        if (JSPLib._active_script && event.data.from !== JSPLib.UID.value) {
            Debug.logLevel(`Storage.broadcastRX[${iteration++}]`, `(${event.data.type}):`, event.data, Debug.INFO);
