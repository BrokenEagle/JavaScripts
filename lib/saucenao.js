...
JSPLib.saucenao.getSauce = async function (image_url,database=null,numres=null,notify_user=false) {
    ...
    try {
        return await JSPLib._jQuery.getJSON(this.query_url,url_addons)
        .always(()=>{
        ...
        let error_key = `${JSPLib._jQuery.param(url_addons)}`;
...
JSPLib.saucenao.checkSauce = function(self,resp) {
    if (!resp) {
        JSPLib.notice.error("SauceNAO server error! (60 second wait)");
        self.debug('errorLevel',"SauceNAO server error!",JSPLib.debug.ERROR);
        this.no_sauce = true;
    ...
    if (resp.header.long_remaining == 0) {
        ...
        self.debug('warnLevel',"No more sauce!",JSPLib.debug.ERROR);
        this.no_sauce = true;
    ...
    if (resp.header.long_remaining > 0 && resp.header.short_remaining == 0) {
        ...
        self.debug('logLevel',"Too much sauce!",JSPLib.debug.WARNING);
    ...
    if (resp.header.status < 0) {
        if (resp.header.message) {
            ...
            self.debug('warnLevel',"Error:",resp.header.message,JSPLib.debug.WARNING);
        }
        return false;
    }
    if (!resp.results) {
        JSPLib.notice.notice("No SauceNao results.");
        self.debug('warnLevel',"No SauceNao results.",JSPLib.debug.WARNING);
        return false;
    }
    return true;
}
...
JSPLib.debug.addModuleLogs('saucenao',['checkSauce']);
