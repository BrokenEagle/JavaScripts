...
Debug.recordKey = function (func_name, parameters) {
    if (Debug.mode) {
        let UID = func_name;
        if (typeof parameters === 'string' || typeof parameters === 'number' || parameters === null) {
            UID += ';' + parameters;
        ...
        UID += ';' + String(Math.floor(Math.random() * Number.MAX_SAFE_INTEGER));
        return UID;
    }
    return null;
};

Debug.recordTime = function (type, record_key) {
    if (Debug.mode) {
        let index = type + ',' + record_key;
        RECORDS[index] = {
            record_key,
            type,
...
Debug.recordTimeEnd = function (type, record_key) {
    if (Debug.mode) {
        let index = type + ',' + record_key;
        if (!(index in RECORDS)) {
            return;
