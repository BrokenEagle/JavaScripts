...
Template.normalizeText = function (text) {
    // Replace carriage returns and spaces with &# equivalents,
    // since the render template will remove all extra whitespace.
    // Once they get inserted into the DOM, they'll go back to normal.
...
function _renderTemplate(literals, args, mapping) {
    ...
    for (let i = 0; i < literals.raw.length; i++) {
        ...
        if (i < args.length) {
            ...
            output += Template.normalizeText(insert);
        }
    }
    return output;
}
...
function _normalizeHTML(output) {
    ...
    let normalized_output = marked_output.replace(/\s+/g, ' ').replace(/(?<=>)\s/g, "").replace(/\s(?=<)/g, "").replaceAll(' >', '>').replace(/\s*&(nb|en|em|thin)sp;\s*/g, '&$1sp;');
