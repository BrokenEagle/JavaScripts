...
Notice.installBanner = function () {
    ...
    jQueryProxy(`#${JSPLib.program_shortcut}-close-notice-link`).on(JSPLib.program_click, _closeNoticeClick);
...
function _closeNoticeClick() {
    jQueryProxy(`#${JSPLib.program_shortcut}-notice`).fadeOut('fast').children('span').html(".");
}
...
function _notice(msg, permanent = false, append = true) {
    ...
    if (!permanent) {
        Notice.notice_timeout_id = setTimeout(() => {
            jQueryProxy(`#${JSPLib.program_shortcut}-close-notice-link`).click();
...
function _processNotice(add_class, remove_class, msg, append) {
    let $notice = jQueryProxy(`#${JSPLib.program_shortcut}-notice`);
