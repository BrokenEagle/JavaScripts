...
(function ({jQueryProxy, Debug, Utility}) {
...
#%SHORTCUT%-close-notice-link,
#%SHORTCUT%-close-notice-link:hover {
    color: #0073ff;
}
div#%SHORTCUT%-notice.ui-state-highlight {
    color: #5f3f3f;
    background-color: #fffbbf;
    border: 1px solid #ccc999;
}
div#%SHORTCUT%-notice.ui-state-error {
    color: #5f3f3f;
...
Notice.installBanner = function () {
    let notice_banner = `<div id="${JSPLib.shortcut}-notice"><span>.</span><a id="${JSPLib.shortcut}-close-notice-link">close</a></div>`;
    let notice_css = Utility.regexReplace(NOTICE_CSS, {SHORTCUT: JSPLib.shortcut});
    Utility.setCSSStyle(notice_css, 'Notice.notice');
    jQueryProxy('body').append(notice_banner);
    jQueryProxy(`#${JSPLib.shortcut}-close-notice-link`).on(JSPLib.event.click, _closeNoticeClick);
...
function _closeNoticeClick() {
    jQueryProxy(`#${JSPLib.shortcut}-notice`).fadeOut('fast').children('span').html(".");
}
...
function _notice(msg, permanent = false, append = true) {
    ...
    if (!permanent) {
        Notice.notice_timeout_id = setTimeout(() => {
            jQueryProxy(`#${JSPLib.shortcut}-close-notice-link`).click();
...
function _processNotice(add_class, remove_class, msg, append) {
    let $notice = jQueryProxy(`#${JSPLib.shortcut}-notice`);
