...
Notice.installBanner = function () {
    ...
    jQuery(`#${JSPLib.program_shortcut}-close-notice-link`).on(JSPLib.program_click, _closeNoticeClick);
...
function _closeNoticeClick() {
    jQuery(`#${JSPLib.program_shortcut}-notice`).fadeOut('fast').children('span').html(".");
}
...
function _notice(msg, permanent = false, append = true) {
    ...
    if (!permanent) {
        Notice.notice_timeout_id = setTimeout(() => {
            jQuery(`#${JSPLib.program_shortcut}-close-notice-link`).click();
...
function _processNotice(add_class, remove_class, msg, append) {
    let $notice = jQuery(`#${JSPLib.program_shortcut}-notice`);
...
})(JSPLib._jQueryProxy, JSPLib.Debug, JSPLib.Utility);
