...
JSPLib.utility.arrayEquals = function (array1,array2) {
    ...
    return [...set1].every(val => set1.has(val));
};
    ...
    new MutationObserver(function(mutations,observer) {
        mutations.forEach(function(mutation) {
            ...
            if (mutation.type == "childList" && mutation.removedNodes.length === 1) {
                ...
                JSPLib.debug.debuglogLevel(`utility.setupMutationRemoveObserver - Checking removed node: ${type} ${name} "${node.className}" "${node.id}"`,JSPLib.debug.DEBUG);
                if ((type === "." && name === node.className) || (type === "#" && name === node.id)) {
                    JSPLib.debug.debuglogLevel(`utility.setupMutationRemoveObserver - Server: ${remove_selector} has been modified!`,JSPLib.debug.INFO);
                    func();
                    observer.disconnect();
                }
            }
