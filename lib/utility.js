...
Utility.deepCopy = function (data) {
    ...
    let entries = (data.constructor.name === 'Set' ? data.entries() : Object.entries(data));
    const copyMethod = (data.constructor.name === 'Set' ? (_, value) => newdata.add(value) : (key, value) => (newdata[key] = value));
...
Utility.assignObjects = function (...objects) {
    return _mergeObjectsRecurse(objects[0], objects.slice(1), false);
};

Utility.mergeObjects = function (...objects) {
    return _mergeObjectsRecurse(Utility.deepCopy(objects[0]), objects.slice(1), true);
};
...
function _mergeObjectsRecurse(merge, objects, clone) {
    for (let i = 0; i < objects.length; i++) {
        for (let k in objects[i]) {
            if (Utility.isHash(merge[k]) && Utility.isHash(objects[i][k])) {
                merge[k] = _mergeObjectsRecurse(merge[k], objects[i][k], clone);
            } else {
                merge[k] = (clone ? Utility.deepCopy(objects[i][k]) : objects[i][k]);
            }
        }
    }
    return merge;
