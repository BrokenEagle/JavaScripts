...
Utility.deepCopy = function (olddata) {
...
Utility.assignObjects = function (...objects) {
    return _mergeObjectsRecurse(objects[0], objects.slice(1));
};

Utility.mergeObjects = function (...objects) {
    return _mergeObjectsRecurse(Utility.deepCopy(objects[0]), objects.slice(1));
};
...
function _mergeObjectsRecurse(merge, objects) {
    for (let i = 0; i < objects.length; i++) {
        for (let k in objects[i]) {
            if (Utility.isHash(merge[k]) && Utility.isHash(objects[i][k])) {
                merge[k] = Utility.mergeObjects(merge[k], objects[i][k]);
            } else {
                merge[k] = Utility.deepCopy(objects[i][k]);
            }
        }
    }
    return merge;
