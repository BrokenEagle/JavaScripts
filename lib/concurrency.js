...
JSPLib.concurrency.checkSemaphore = function (program_shortcut,name) {
    let storage_name = JSPLib.concurrency._getSemaphoreName(program_shortcut,name,true);
    let semaphore = JSPLib.storage.getStorageData(storage_name,localStorage,0);
    return !JSPLib.utility.validateExpires(semaphore, JSPLib.concurrency.process_semaphore_expires);
};

JSPLib.concurrency.freeSemaphore = function (program_shortcut,name) {
    let event_name = JSPLib.concurrency._getSemaphoreName(program_shortcut,name,false);
    let storage_name = JSPLib.concurrency._getSemaphoreName(program_shortcut,name,true);
...
JSPLib.concurrency.reserveSemaphore = function (program_shortcut,name) {
    ...
    if (JSPLib.concurrency.checkSemaphore(program_shortcut,name)) {
        ...
        let semaphore = JSPLib.utility.getExpires(JSPLib.concurrency.process_semaphore_expires);
