...
Validate.nonnegative_integer_constraints = {
    integer: {
        greaterThanOrEqual: 0,
    },
};

Validate.positive_integer_constraints = {
    integer: {
        greaterThan: 0,
    },
};
...
Validate.basic_number_validator = {
    func: Utility.isNumber,
    type: 'number',
};

Validate.basic_integer_validator = {
    func: Utility.isInteger,
    type: 'integer',
};
...
Validate.tagentryarray_constraints = function(categories) {
    let option = (Utility.isArray(categories) ? {categories} : true);
    return {
        presence: true,
...
Validate.validateArrayValues = function(key, array, validator) {
    ...
    if (!Utility.isArray(array)) {
        printer.logLevel(`${key} is not an array.`, Debug.INFO);
        return false;
    }
...
Validate.outputValidateError = function (key, checkerror) {
    const printer = Debug.getFunctionPrint('Validate.outputValidateError');
    let error_text = JSON.stringify(checkerror, null, 2);
    printer.logLevel(key, ':\r\n', error_text, Debug.INFO);
    _executeIfOutput(($domobj) => {
        let output_text = `<b>${key}:</b><br><pre>${error_text}</pre>`;
...
if ((typeof ValidateJS === 'function') && (typeof ValidateJS.validators === 'object')) {
    // Debug mode/level and program shortcut will be set during the synchronous portion, so check it asynchronously.
    Utility.sleep(1).then(() => {
        Debug.logLevel("Validate.js detected... custom validators installed.", Debug.DEBUG);
    });
    ...
    ValidateJS.validators.string = function(value, options) {
        if (options !== false) {
            ...
            if (_checkOptions(options, 'allowNull')) {
    ...
    ValidateJS.validators.integer = function(value, options) {
        ...
        } else if (_checkOptions(options, 'greaterThan') && options.greaterThan >= value) {
            message = "is not greater than " + options.greaterThan;
        } else if (_checkOptions(options, 'greaterThanOrEqual') && options.greaterThanOrEqual > value) {
            message = "is not greater than or equal to " + options.greaterThanOrEqual;
        } else if (_checkOptions(options, 'lessThan') && options.lessThan <= value) {
            message = "is not less than " + options.lessThan;
        } else if (_checkOptions(options, 'lessThanOrEqual') && options.lessThanOrEqual < value) {
    ...
    ValidateJS.validators.array = function(value, options, key) {
        if (options !== false) {
            ...
            if (_checkOptions(options, 'length')) {
                ...
                let validator = Utility.mergeObjects(options.length, usage_messages);
                let array_key = _sanitizeKey(key);
    ...
    ValidateJS.validators.tagentryarray = function(value, options) {
        if (options !== false) {
            ...
            let categories = (_checkOptions(options, 'categories') ? options.categories : TAG_CATEGORIES);
...
function _checkOptions(options, key) {
