...
JSPLib.utility.arrayDifference = JSPLib.utility.arrayDifference || (()=>{return [];});
...
JSPLib.validate.validateIDList = function (array) {
    return Array.isArray(array) && ((array.length === 0) || ((array.length > 0) && array.reduce((total,val)=>{return this.validateID(val) && total;},true)));
};
...
JSPLib.validate._configuration = {
    nonenumerable: ['_executeIfOutput','_sanitizeKey','_configuration'],
    nonwritable: ['_configuration']
};
Object.defineProperty(JSPLib,'validate',{configurable:false,writable:false});
for (let property in JSPLib.validate) {
    if (JSPLib.validate._configuration.nonenumerable.includes(property)) {
        Object.defineProperty(JSPLib.validate,property,{enumerable:false});
    }
    if (JSPLib.validate._configuration.nonwritable.includes(property)) {
        Object.defineProperty(JSPLib.validate,property,{writable:false});
    }
    Object.defineProperty(JSPLib.validate,property,{configurable:false});
}
