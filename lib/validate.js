...
JSPLib.validate.printValidateError = function (key, checkerror) {
    const printer = JSPLib.debug.getFunctionPrint('validate.printValidateError');
...
JSPLib.validate.validateArrayValues = function(key, array, validator) {
    const printer = JSPLib.debug.getFunctionPrint('validate.validateArrayValues');
    for (let i = 0;i < array.length; i++) {
        if (!validator.func(array[i])) {
            let display_key = `${key}[${i}]`;
            let display_item = JSON.stringify(array[i]);
            printer.debuglogLevel(`"${display_key}" ${display_item} is not a valid ${validator.type}.`, JSPLib.debug.INFO);
            return false;
        }
    }
    return true;
...
JSPLib.validate.validateHashValues = function(parent_key, hash, validator) {
    const printer = JSPLib.debug.getFunctionPrint('validate.validateHashValues');
    for (let key in hash) {
