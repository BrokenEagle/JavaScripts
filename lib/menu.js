...
JSPLib.menu.saveMenuHotkey = function (event) {
    jQuery('#userscript-settings-sections [aria-hidden=false] .jsplib-settings-buttons [id$="-commit"]').click();
};

JSPLib.menu.switchMenuHotkey = function (event) {
    let menu_number = (event.originalEvent.key === "0" ? "10" : event.originalEvent.key);
    jQuery(`#userscript-settings-tabs li:nth-of-type(${menu_number}) a`).click()
};
...
JSPLib.menu.getEnabledDomains = function () {
    ...
    let valid_domains = JSPLib.utility.arrayIntersection(this.domains,cookie_domains);
    ...
    for (let setting in config) {
        if (!(setting in settings) || !config[setting].validate(settings[setting])) {
            ...
            if (Array.isArray(config[setting].allitems) && Array.isArray(settings[setting])) {
                settings[setting] = JSPLib.utility.arrayDifference(config[setting].allitems,settings[setting]);
...
JSPLib.menu.saveUserSettingsClick = function (local_callback=null) {
    ...
    jQuery(`#${program_shortcut}-commit`).on(this.program_click,(event)=>{
        ...
        if (jQuery(`#${program_shortcut}-settings .${program_shortcut}-global`).length) {
            ...
            let enabled_domains = JSPLib.utility.arrayDifference(selected_domains,this._current_domains);
            let disabled_domains = JSPLib.utility.arrayDifference(this._current_domains,selected_domains);
...
JSPLib.menu.listCacheClick = function () {
    let program_data = this.program_data;
    let program_shortcut = this.program_shortcut;
    jQuery(`#${this.program_shortcut}-data-name-list`).on(this.program_click,(event)=>{
        this.getCacheDatakey();
        if (!this._storage_keys_loaded) {
            JSPLib.utility.notice("Waiting for keys to load...");
        }
        this._storage_keys_promise.then(()=>{
            jQuery(`#${program_shortcut}-cache-viewer textarea`).val(JSON.stringify(program_data.storage_keys[program_data.data_source],null,2));
        });
    });
};

JSPLib.menu.refreshCacheClick = function () {
    jQuery(`#${this.program_shortcut}-data-name-refresh`).on(this.program_click,(event)=>{
        this.loadStorageKeys().then(()=>{
            JSPLib.utility.notice("Data names have been refreshed.");
        });
    });
};
...
JSPLib.menu._broadcastRX = function () {
    ...
    return function (event) {
        ...
        JSPLib.debug.debuglogLevel('menu [broadcast] -',`(${event.data.type}):`,event.data,JSPLib.debug.INFO);
...
JSPLib.menu._configuration = {
    nonenumerable: ['_purge_is_started','_data_name_disabled','_storage_keys_loaded','_storage_keys_promise','_active_script','_window','_channel','_current_subdomain','_current_domains','_isSettingMenu','_broadcastRX','_configuration'],
