...
<div id="userscript-settings-menu" class="tab-panel-component horizontal-tab-panel" x-data="{ active: 0 }" data-version="${JSPLib.menu.version}">
    <div class="tab-list thin-x-scrollbar">
    </div>
...
<a x-on:click.prevent="active = ${'index'}" x-bind:class="{ 'active-tab': active === ${'index'} }" href="#" class="tab">
    ${'text'}
</a>`;

const SECTION_TEMPLATE = utility.normalizeHTML({template: true})`
<div x-bind:class="{ 'active-tab': active === ${'index'} }" class="tab-panel">
    ${'html'}
</div>`;

const TEXTINPUT_TEMPLATE = utility.normalizeHTML({template: true})`
<div class="${'shortcut'}-textinput jsplib-textinput jsplib-menu-item" data-setting="${'setting_name'}" data-type="text">
    <h4>${'display_name'}</h4>
    <div>
        <input type="text" class="${'shortcut'}-${'menu_type'} jsplib-${'menu_type'}" name="${'textinput_key'}" id="${'textinput_key'}" value="${'value'}" size="${'length'}" autocomplete="off" data-parent="2">
        ${'submit_control'}
        ${'hint'}
    </div>
</div>`;
...
<div class="${'shortcut'}-checkbox jsplib-checkbox jsplib-menu-item" data-setting="${'setting_name'}" data-type="checkbox">
    <h4>${'display_name'}</h4>
    <div>
        <input type="checkbox" ${'checked'} class="${'shortcut'}-${'menu_type'} jsplib-${'menu_type'}" name="${'shortcut'}-enable-${'setting_key'}" id="${'shortcut'}-enable-${'setting_key'}" data-parent="2">
        ${'hint'}
    </div>
</div>`;

const SORTLIST_TEMPLATE = utility.normalizeHTML({template: true})`
<div class="${'shortcut'}-sortlist jsplib-sortlist jsplib-menu-item" data-setting="${'setting_name'}" data-type="sortlist">
    <h4>${'display_name'}</h4>
    <div>
        <ul>
            ${'html'}
...
<li class="ui-state-default">
    <input type="hidden" class="${'shortcut'}-setting jsplib-setting" name="${'sortitem_key'}" id="${'sortitem_key'}" data-sort="${'sortitem'}" data-parent="4">
    <div>
...
<div class="${'shortcut'}-selectors jsplib-selectors jsplib-menu-item" data-setting="${'setting_name'}" data-type="selector">
    <h4>${'display_name'}</h4>
    <div>
        ${'html'}
        ${'submit_control'}
...
<input type="${'type'}" ${'checked'} class="${'shortcut'}-${'menu_type'} jsplib-${'menu_type'}" name="${'name'}" id="${'key'}" data-selector="${'selector'}" data-parent="2">`;

const DOMAIN_TEMPLATE = utility.normalizeHTML({template: true})`
<div class="${'shortcut'}-selectors jsplib-selectors jsplib-menu-item" data-setting="domain_selector">
...
<label for="${'key'}">${'display'}</label>
<input type="checkbox" ${'checked'} class="${'shortcut'}-global jsplib-global" name="${'name'}" id="${'selection_key'}" data-selector="${'selector'}" data-parent="2">`;
...
JSPLib.menu.renderKeyselect = function (setting_name, is_control = false) {
    ...
    config[setting_name].allitems.forEach((option) => {
        ...
        let display_option = utility.displayCase(option);
...
JSPLib.menu.renderDataSourceSections = function () {
    ...
    allitems.forEach((source) => {
        ...
        let source_key = `${JSPLib.program_shortcut}-section-${utility.kebabCase(source)}`;
...
JSPLib.menu.renderControlButton = function (setting_key, button_name, parent_level) {
    ...
    let display_name = utility.displayCase(button_name);
...
JSPLib.menu.renderMenuSection = function (value, type) {
    ...
    let section_key = utility.kebabCase(value.name);
    let section_name = utility.displayCase(value.name);
    return `
...
JSPLib.menu.getCheckboxRadioSelected = function (selector) {
    return jQuery(selector).map((_, input) => (input.checked ? jQuery(input).data('selector') : undefined)).toArray();
};
...
JSPLib.menu.validateCheckboxRadio = function (data, type, allitems) {
    return Array.isArray(data)
        ...
        && utility.isSubArray(allitems, data)
        && (type !== 'radio' || data.length === 1);
};

JSPLib.menu.validateNumber = function (data, is_integer, min, max) {
    const validator = (is_integer ? Number.isInteger : utility.isNumber);
    min = min || -Infinity;
    max = max || Infinity;
...
JSPLib.menu.updateGlobalSettings = function () {
    ...
    jQuery(`#${JSPLib.program_shortcut}-settings .${JSPLib.program_shortcut}-global[id]`).each((_, entry) => {
        let $input = jQuery(entry);
        let parent_level = $input.data('parent');
        let container = utility.getNthParent(entry, parent_level);
...
JSPLib.menu.purgeCacheClick = function () {
    jQuery(`#${JSPLib.program_shortcut}-control-purge-cache`).on(JSPLib.program.click, (event) => {
...
JSPLib.menu.cacheInfoClick = function () {
    jQuery(`#${JSPLib.program_shortcut}-control-cache-info`).on(JSPLib.program.click, (event) => {
...
`;
            jQuery(`#${JSPLib.program_shortcut}-cache-info-table`).html(html).show();
        });
        event.preventDefault();
    });
};
...
JSPLib.menu._collectSettingsInputs = function (config, settings) {
    let invalid_setting = false;
    let temp_selectors = {};
    let settings_inputs = jQuery(`#${JSPLib.program_shortcut}-settings .${JSPLib.program_shortcut}-setting[id]`);
