...
JSPLib.menu.installSettingsMenu = function (self) {
    if (JSPLib._jQuery("#userscript-settings-menu").length === 0) {
        ...
        JSPLib._jQuery('.tab-panels').append(this.settings_field);
        JSPLib._jQuery('.tab-list').append('<a id="userscript-menu-link" href="#userscript-menu">Userscript Menus</a>');
        ...
        JSPLib._jQuery('.tab-panel-component').css('max-width', '100%');
        JSPLib._jQuery('.tab-list, .security-tab').css('max-width', '60rem');
        ...
        if (version < this.version) {
            ...
            let settings_tab = JSPLib._jQuery('#userscript-menu-link').detach();
            let settings_section = JSPLib._jQuery('#userscript-settings-menu').detach();
            //Multiple sections with the same ID get added due to the recent settings menu change
            JSPLib._jQuery('#userscript-settings-menu').each((i, entry) => JSPLib._jQuery(entry).remove());
            JSPLib._jQuery('.tab-list').append(settings_tab);
            JSPLib._jQuery('.tab-panel-component').append(settings_section);
            //Manually redo the settings click
            JSPLib._jQuery(this.settings_selector).off('click.jsplib');
            JSPLib._jQuery(this.other_selectors).off('click.jsplib');
            ...
            JSPLib._jQuery('style').filter((i, entry) => entry.textContent.match(/#userscript-menu-link/)).remove();
            JSPLib._jQuery('style').filter((i, entry) => entry.textContent.match(/\/\*\*\*JQUERY-UI\*\*\*\//)).remove();
...
JSPLib.menu.cacheSource = function () {
    ...
    return function (req, resp) {
        ...
        if (program_data.data_source === 'local_storage') {
            transformed_keys = this._keyToNameTransform(transformed_keys, context.program_shortcut);
        } else if (program_data.data_type !== "custom") {
            let program_keys = transformed_keys.filter((key) => key.match(context.program_regex));
            let program_names = this._keyToNameTransform(program_keys, context.program_shortcut);
