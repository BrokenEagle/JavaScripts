...
Menu.renderTextinput = function (setting_name, length = 20, is_control = false) {
    ...
    let submit_control = "";
    if (is_control) {
        submit_control = config[setting_name].buttons.map((button) => _renderControlButton(setting_key, button, 2)).join("");
    }
...
Menu.resetUserSettingsClick = function ({delete_keys = [], local_callback = null} = {}) {
    jQueryProxy(`#${JSPLib.shortcut}-resetall`).on(JSPLib.event.click, () => {
        ...
        if (confirm(`This will reset all of ${JSPLib.name}'s settings.\n\nAre you sure?`)) {
            ...
            JSPLib.data = Utility.mergeObjects(JSPLib.data, JSPLib.reset_data, {storage_keys: {local_storage: []}});
            Storage.setLocalData(`${JSPLib.shortcut}-user-settings`, settings);
        ...
        if (JSPLib.data.data_source === "local_storage") {
            if (JSPLib.data.raw_data) {
                ...
                for (let key in data) {
                    ...
                    if (!_saveLocalData(data_key, data[key], localvalidator, localupdater)) {
            ...
            } else if (_saveLocalData(storage_key, data, localvalidator, localupdater)) {
...
function _renderControlButton(setting_key, button_name) {
...
function _saveLocalData(key, data, validator, localupdater) {
...
function _broadcastReset(event) {
    JSPLib.data = Utility.mergeObjects(JSPLib.data, JSPLib.reset_data);
    _loadStorageKeys();
