...
Menu.installSettingsMenu = function () {
    ...
    if (jQueryProxy("#userscript-settings-menu").length === 0) {
        ...
        jQueryProxy(document).on(JSPLib.program.keydown, null, 'alt+s', Menu.saveMenuHotkey);
    }
    ...
    settings_tabs.push(jQueryProxy(`<span class="jsplib-tab" data-name="${JSPLib.program_id}">${JSPLib.program_name}</span>`).get(0));
    settings_panels.push(jQueryProxy(`<div id="${JSPLib.program_id}" class="jsplib-section" data-name="${JSPLib.program_id}"></div>`).get(0));
...
Menu.engageUI = function ({checkboxradio = false, sortable = false} = {}) {
    if (checkboxradio) {
        jQueryProxy(`.${JSPLib.program_shortcut}-selectors input`).checkboxradio();
    }
    if (sortable) {
        jQueryProxy(`.${JSPLib.program_shortcut}-sortlist ul`).sortable();
    }
...
Menu.updateUserSettings = function () {
    ...
    jQueryProxy(`#${JSPLib.program_shortcut}-settings [data-setting][data-type]`).each((_, entry) => {
        ...
        switch (type) {
            ...
            case 'sortlist':
                ...
                jQueryProxy("li", entry).detach().sort((a, b) => {
                    ...
                    jQueryProxy("ul", entry).append(entry);
                });
...
Menu.updateGlobalSettings = function () {
    ...
    jQueryProxy(`#${JSPLib.program_shortcut}-settings .jsplib-selectors[data-setting="domain_selector"] input`).each((_, entry) => {
...
Menu.saveUserSettingsClick = function ({local_callback = null} = {}) {
    jQueryProxy(`#${JSPLib.program_shortcut}-commit`).on(JSPLib.program.click, () => {
        ...
        printer.logLevel("Settings updated:", {new_settings: settings, old_settings: JSPLib.program_data.old_settings, invalid_setting}, Debug.VERBOSE);
        Storage.setLocalData(`${JSPLib.program_shortcut}-user-settings`, settings);
        CHANNEL.postMessage({type: 'settings', program_shortcut: JSPLib.program_shortcut, from: JSPLib.UID.value, user_settings: settings});
        if (!invalid_setting) {
            Notice.notice(`<b>${JSPLib.program_name}:</b> Settings updated!`);
        } else {
        ...
        let $domain_inputs = jQueryProxy(`#${JSPLib.program_shortcut}-settings .${JSPLib.program_shortcut}-global`);
...
Menu.resetUserSettingsClick = function ({delete_keys = [], local_callback = null} = {}) {
    jQueryProxy(`#${JSPLib.program_shortcut}-resetall`).on(JSPLib.program.click, () => {
        let config = Menu.settings_config;
        let settings = JSPLib.program_data.user_settings;
        JSPLib.program_data.old_settings = Utility.dataCopy(settings);
        if (confirm(`This will reset all of ${JSPLib.program_name}'s settings.\n\nAre you sure?`)) {
            ...
            Notice.notice(`<b>${JSPLib.program_name}:</b> Settings reset to defaults!`);
            if (typeof local_callback === 'function') {
                local_callback();
            }
        }
...
Menu.purgeCacheClick = function () {
    jQueryProxy(`#${JSPLib.program_shortcut}-control-purge-cache`).on(JSPLib.program.click, () => {
...
Menu.cacheInfoClick = function () {
    jQueryProxy(`#${JSPLib.program_shortcut}-control-cache-info`).on(JSPLib.program.click, () => {
        ...
        Storage.programCacheInfo(Menu.program_data_regex).then((cache_info) => {
            ...
            jQueryProxy(`#${JSPLib.program_shortcut}-cache-info-table`).html(CACHE_INFO_TEMPLATE(template_data)).show();
        });
    });
};

Menu.expandableClick = function () {
    jQueryProxy(`.${JSPLib.program_shortcut}-expandable .jsplib-expandable-button`).on(JSPLib.program.click, (event) => {
...
Menu.getCacheDatakey = function () {
    JSPLib.program_data.data_source = jQueryProxy(`#${JSPLib.program_shortcut}-control-data-source`).val();
    JSPLib.program_data.data_type = jQueryProxy(`#${JSPLib.program_shortcut}-control-data-type`).val();
    let data_key = JSPLib.program_data.data_value = jQueryProxy(`#${JSPLib.program_shortcut}-control-data-name`).val().trim();
    if (JSPLib.program_data.data_source === "local_storage") {
        JSPLib.program_data.raw_data = jQueryProxy(`#${JSPLib.program_shortcut}-enable-raw-data`).prop('checked');
...
Menu.adjust_data_name = function (disable = true) {
    ...
    if (disable && DATA_NAME_DISABLED) {
        jQueryProxy(name_selector).val("");
        jQueryProxy(name_selector).prop('disabled', true);
    } else {
        jQueryProxy(name_selector).prop('disabled', false);
    }
};

Menu.dataSourceChange = function () {
    jQueryProxy(`#${JSPLib.program_shortcut}-control-data-source`).on(JSPLib.program.change, () => {
        let data_source = jQueryProxy(`#${JSPLib.program_shortcut}-control-data-source`).val();
        jQueryProxy(`.${JSPLib.program_shortcut}-section-data-source`).hide();
...
Menu.rawDataChange = function () {
    jQueryProxy(`#${JSPLib.program_shortcut}-enable-raw-data`).on(JSPLib.program.change, (event) => {
...
Menu.getCacheClick = function (localvalidator) {
    jQueryProxy(`#${JSPLib.program_shortcut}-data-name-get`).on(JSPLib.program.click, () => {
        ...
        if (JSPLib.program_data.data_source === "local_storage") {
            ...
            jQueryProxy(`#${JSPLib.program_shortcut}-cache-viewer textarea`).val(JSON.stringify(data, null, 2));
        } else {
...
Menu.saveCacheClick = function (localvalidator, indexvalidator, localupdater) {
    jQueryProxy(`#${JSPLib.program_shortcut}-data-name-save`).on(JSPLib.program.click, () => {
        var data;
        try {
            data = JSON.parse(jQueryProxy(`#${JSPLib.program_shortcut}-cache-viewer textarea`).val());
        } catch (error) {
        ...
        if (JSPLib.program_data.data_source === "local_storage") {
            if (JSPLib.program_data.raw_data) {
                let error_messages = [];
                let $cache_errors = jQueryProxy(`#${JSPLib.program_shortcut}-cache-editor-errors`);
                for (let key in data) {
...
Menu.deleteCacheClick = function () {
    jQueryProxy(`#${JSPLib.program_shortcut}-data-name-delete`).on(JSPLib.program.click, () => {
        let storage_key = Menu.getCacheDatakey();
        if (JSPLib.program_data.data_source === "local_storage") {
...
Menu.listCacheClick = function () {
    jQueryProxy(`#${JSPLib.program_shortcut}-data-name-list`).on(JSPLib.program.click, () => {
        Menu.getCacheDatakey();
        if (!STORAGE_KEYS_LOADED) {
            Notice.notice("Waiting for keys to load...");
        }
        STORAGE_KEYS_PROMISE.then(() => {
            jQueryProxy(`#${JSPLib.program_shortcut}-cache-viewer textarea`).val(JSON.stringify(JSPLib.program_data.storage_keys[JSPLib.program_data.data_source], null, 2));
        });
    });
};

Menu.refreshCacheClick = function () {
    jQueryProxy(`#${JSPLib.program_shortcut}-data-name-refresh`).on(JSPLib.program.click, () => {
...
Menu.cacheAutocomplete = function () {
    let $control_data = jQueryProxy(`#${JSPLib.program_shortcut}-control-data-name`);
...
function _broadcastRX() {
    ...
    return function (event) {
        ...
        printer.logLevel(`Menu.broadcastRX[${iteration++}]`, `(${event.data.type}):`, event.data, Debug.INFO);
