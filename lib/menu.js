...
Menu.renderControlButton = function (setting_key, button_name) {
    return CONTROL_BUTTON_TEMPLATE({
        ...
        key: `${JSPLib.shortcut}-${setting_key}-${button_name}`,
    });
};
...
Menu.updateUserSettings = function () {
    ...
    jQueryProxy(`#${JSPLib.shortcut}-settings [data-setting][data-type]`).each((_, entry) => {
...
Menu.saveUserSettingsClick = function ({local_callback = null} = {}) {
    jQueryProxy(`#${JSPLib.shortcut}-commit`).on(JSPLib.event.click, () => {
        ...
        Storage.setLocalData(`${JSPLib.shortcut}-user-settings`, settings);
        CHANNEL.postMessage({type: 'settings', program_shortcut: JSPLib.shortcut, from: JSPLib.UID.value, user_settings: settings});
        if (!invalid_setting) {
        ...
        if ($domain_inputs.length) {
            ...
            if (!Utility.arrayEquals(CURRENT_DOMAINS, selected_domains)) {
                ...
                if (enabled_domains.length || disabled_domains.length) {
                    CHANNEL.postMessage({type: 'domain', program_shortcut: JSPLib.shortcut, from: JSPLib.UID.value, enabled_domains, disabled_domains, current_domains: selected_domains});
                }
...
Menu.resetUserSettingsClick = function ({delete_keys = [], local_callback = null} = {}) {
    jQueryProxy(`#${JSPLib.shortcut}-resetall`).on(JSPLib.event.click, () => {
        ...
        if (confirm(`This will reset all of ${JSPLib.name}'s settings.\n\nAre you sure?`)) {
            ...
            Object.assign(JSPLib.data, Utility.dataCopy(Menu.program_reset_data), {storage_keys: {local_storage: []}});
            Storage.setLocalData(`${JSPLib.shortcut}-user-settings`, settings);
            CHANNEL.postMessage({type: 'reset', program_shortcut: JSPLib.shortcut, from: JSPLib.UID.value, user_settings: settings});
...
Menu.getCacheDatakey = function () {
    ...
    let data_key = JSPLib.data.data_value = jQueryProxy(`#${JSPLib.shortcut}-control-data-name`).val().trim();
    if (JSPLib.data.data_source === "local_storage") {
        JSPLib.data.raw_data = jQueryProxy(`#${JSPLib.shortcut}-enable-raw-data`).prop('checked');
        data_key = JSPLib.shortcut + '-' + JSPLib.data.data_value;
    } else if (JSPLib.data.data_type !== "custom") {
        if (typeof Menu.program_data_key === "function") {
            data_key = Menu.program_data_key(JSPLib.data.data_type, JSPLib.data.data_value);
...
Menu.saveLocalData = function (key, data, validator, localupdater) {
    if (validator(key, data)) {
        ...
        if (key === `${JSPLib.shortcut}-user-settings`) {
            ...
            Menu.updateUserSettings();
            CHANNEL.postMessage({type: 'settings', program_shortcut: JSPLib.shortcut, from: JSPLib.UID.value, user_settings: data});
...
Menu.adjust_data_name = function (disable = true) {
    let name_selector = `#${JSPLib.shortcut}-control-data-name`;
...
Menu.getCacheClick = function (localvalidator) {
    jQueryProxy(`#${JSPLib.shortcut}-data-name-get`).on(JSPLib.event.click, () => {
...
Menu.saveCacheClick = function (localvalidator, indexvalidator, localupdater) {
    jQueryProxy(`#${JSPLib.shortcut}-data-name-save`).on(JSPLib.event.click, () => {
        var data;
        try {
...
Menu.deleteCacheClick = function () {
    jQueryProxy(`#${JSPLib.shortcut}-data-name-delete`).on(JSPLib.event.click, () => {
        let storage_key = Menu.getCacheDatakey();
        if (JSPLib.data.data_source === "local_storage") {
...
Menu.listCacheClick = function () {
    jQueryProxy(`#${JSPLib.shortcut}-data-name-list`).on(JSPLib.event.click, () => {
        Menu.getCacheDatakey();
        if (!STORAGE_KEYS_LOADED) {
            Notice.notice("Waiting for keys to load...");
        }
        STORAGE_KEYS_PROMISE.then(() => {
            jQueryProxy(`#${JSPLib.shortcut}-cache-viewer textarea`).val(JSON.stringify(JSPLib.data.storage_keys[JSPLib.data.data_source], null, 2));
        });
    });
};

Menu.refreshCacheClick = function () {
    jQueryProxy(`#${JSPLib.shortcut}-data-name-refresh`).on(JSPLib.event.click, () => {
...
Menu.cacheSource = function () {
    return function (_, resp) {
        ...
        if ((JSPLib.data.data_source === "indexed_db" && JSPLib.data.data_value.length === 0) || !STORAGE_KEYS_LOADED) {
            resp([]);
            return;
        }
...
function _broadcastRX() {
    ...
    return function (event) {
        if (event.data.program_shortcut !== JSPLib.shortcut) {
            return;
        }
