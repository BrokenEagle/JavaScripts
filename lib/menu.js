...
JSPLib.menu.domains = ['danbooru','kagamihara','saitou','shima'];
...
JSPLib.menu.mainSettingsClick = function () {
    if (!JSPLib.utility.isNamespaceBound(JSPLib.menu.settings_selector,'click','jsplib')) {
        JSPLib.debug.debuglogLevel("Installing main setting click",JSPLib.debug.DEBUG);
        jQuery(JSPLib.menu.settings_selector).on('click.jsplib',(e)=>{
...
JSPLib.menu.otherSettingsClicks = function () {
    if (!JSPLib.utility.isNamespaceBound(JSPLib.menu.other_selectors,'click','jsplib')) {
        JSPLib.debug.debuglogLevel("Installing oether setting click",JSPLib.debug.DEBUG);
        jQuery(JSPLib.menu.other_selectors).on('click.jsplib',(e)=>{
...
JSPLib.menu.installSettingsMenu = function (program_name) {
    ...
    if (jQuery("#userscript-settings-menu").length === 0) {
        ...
        let version = jQuery("#userscript-settings-menu").data('version');
        if (version !== JSPLib.menu.version) {
            JSPLib.debug.debuglogLevel("JSPLib.menu: Lower menu version installed", version || 0, ", installing version", JSPLib.menu.version, JSPLib.debug.INFO);
            JSPLib.utility.setCSSStyle(JSPLib.menu.settings_css,'settings');
            jQuery("#userscript-settings-menu").attr('data-version',JSPLib.menu.version);
        }
    }
...
JSPLib.menu.renderDomainSelectors = function (program_shortcut,program_name) {
    let selection_name = `${program_shortcut}-domain-selector`;
    let enabled_selectors = JSPLib.menu.getEnabledDomains(program_name);
    let hint_html = JSPLib.menu.renderSettingHint(program_shortcut,"block","Select which domain the script should be active on.");
    let html = "";
    JSPLib.menu.domains.forEach((selector)=>{
        let checked = (enabled_selectors.includes(selector) ? "checked" : "");
        let display_selection = JSPLib.utility.displayCase(selector);
...
JSPLib.menu.getEnabledDomains = function (program_name) {
    let domains = JSPLib.utility.readCookie(program_name);
    if (!domains) {
        return JSPLib.menu.domains;
    }
    let cookie_domains = domains.split(',');
    let valid_domains = JSPLib.utility.setIntersection(JSPLib.menu.domains,cookie_domains);
    if (valid_domains.length === 0) {
        JSPLib.debug.debuglogLevel("Invalid cookie found!",JSPLib.debug.WARNING);
        JSPLib.utility.eraseCookie(program_name);
        return JSPLib.menu.domains;
    } else if (valid_domains.length != cookie_domains.length) {
        JSPLib.debug.debuglogLevel("Invalid domains found on cookie!",JSPLib.debug.WARNING);
        JSPLib.utility.createCookie(program_name,valid_domains.join(','),null,'donmai.us');
    }
    return valid_domains;
}

JSPLib.menu.isScriptEnabled = function (program_name) {
    let current_subdomain = JSPLib.utility.findAll(window.location.hostname,/^[^.]+/)[0];
    let enabled_subdomains = JSPLib.menu.getEnabledDomains(program_name);
...
JSPLib.menu.hasSettingChanged = function (program_shortcut,setting_name) {
    let program_key = program_shortcut.toUpperCase();
    return JSON.stringify(Danbooru[program_key].user_settings[setting_name]) !== JSON.stringify(Danbooru[program_key].old_settings[setting_name]);
};
...
JSPLib.menu.saveUserSettingsClick = function (program_shortcut,program_name,callback) {
    ...
    jQuery(`#${program_shortcut}-commit`).click((e)=>{
        ...
        if (jQuery(`#${program_shortcut}-settings .${program_shortcut}-global`).length) {
            let selected_domains = JSPLib.menu.getCheckboxRadioSelected(`#${program_shortcut}-settings .${program_shortcut}-global`);
            if (JSPLib.utility.isSuperset(JSPLib.menu.domains,selected_domains)) {
                //Don't bother storing a cookie if all of the domains are active
                JSPLib.utility.eraseCookie(program_name);
            } else {
                JSPLib.utility.createCookie(program_name,selected_domains.join(','),null,'donmai.us');
            }
        }
        if (!invalid_setting) {
...
JSPLib.menu.purgeCacheClick = function (program_shortcut,program_name,regex,counter_domname) {
    ...
    jQuery(`#${program_shortcut}-control-purge-cache`).click((e)=>{
        if (!JSPLib.menu._purge_is_started && confirm(`This will delete all of ${program_name}'s cached data.\n\nAre you sure?`)) {
            JSPLib.menu._purge_is_started = true;
...
JSPLib.menu._configuration = {
    nonenumerable: ['_purge_is_started','_configuration'],
    nonwritable: ['css_themes_url','settings_css','settings_field','settings_selector','_configuration']
};
