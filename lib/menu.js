...
JSPLib.menu.initializeSettingsMenu = function (self, RenderSettingsMenu, menu_CSS) {
    ...
    if (JSPLib.menu.css_debug_url !== null) {
        if (JSPLib.network && !JSPLib.utility.getPublicData(document.body).jsplibDebugCss) {
            css_promise = JSPLib.network.getNotify(JSPLib.menu.css_debug_url, {}, "Unable to load debug CSS.")
                .then((data)=>{
            ...
            css_promise = new Promise((resolve)=>{
                 JSPLib.utility.recheckTimer({
                     check: ()=>{
                         return JSPLib.utility.getPublicData(document.body).jsplibDebugCss && JSPLib.storage.getStorageData('jsplib-debug-css', sessionStorage);
                     },
                     exec: ()=>{
...
JSPLib.debug.addModuleLogs('menu',['mainSettingsClick','otherSettingsClicks','installSettingsMenu','initializeSettingsMenu','getEnabledDomains','loadUserSettings','validateUserSettings']);
