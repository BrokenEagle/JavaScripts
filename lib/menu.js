...
JSPLib.menu.saveUserSettingsClick = function (program_shortcut,program_name,callback) {
    ...
    jQuery(`#${program_shortcut}-commit`).click((e)=>{
        ...
        if (!invalid_setting) {
            JSPLib.menu._notice(`${program_name}: Settings updated!`);
        } else {
            JSPLib.menu._error("Error: Some settings were invalid!")
        }
        callback && callback();
    });
};
...
JSPLib.menu.resetUserSettingsClick = function (program_shortcut,program_name,delete_keys,reset_settings,callback) {
    ...
    jQuery(`#${program_shortcut}-resetall`).click((e)=>{
        ...
        if (confirm(`This will reset all of ${program_name}'s settings.\n\nAre you sure?`)) {
            ...
            JSPLib.menu._notice(`${program_name}: Settings reset to defaults!`);
        }
        callback && callback();
    });
};
...
JSPLib.menu.cacheInfoClick = function (program_shortcut,regex,info_domname) {
    ...
    jQuery(`#${program_shortcut}-control-cache-info`).click((e)=>{
        JSPLib.menu._notice("Calculating cache information...");
...
JSPLib.menu.saveCacheClick = function (program_shortcut,localvalidator,indexvalidator,option,localupdater) {
    ...
    jQuery(`#${program_shortcut}-data-name-save`).on(`click.${program_shortcut}`,(e)=>{
        try {
            ...
            JSPLib.menu._error("Invalid JSON data! Unable to save.");
            return;
        }
        let storage_key = JSPLib.menu.getCacheDatakey(program_shortcut,option);
        if (Danbooru[program_key].data_source === "local_storage") {
            if (localvalidator(storage_key,data)) {
                ...
                JSPLib.menu._notice("Data was saved.");
                JSPLib.validate.hideValidateError();
                ...
                JSPLib.menu._error("Data is invalid! Unable to save.");
            }
        } else {
    ...
    jQuery(`#${program_shortcut}-data-name-delete`).on(`click.${program_shortcut}`,(e)=>{
        ...
        if (Danbooru[program_key].data_source === "local_storage") {
            if (confirm("This will delete program data that may cause problems until the page can be refreshed.\n\nAre you sure?")) {
                ...
                JSPLib.menu._notice("Data has been deleted.");
                JSPLib.validate.hideValidateError();
            }
        } else {
...
JSPLib.menu._notice = function (message) {
    window.Danbooru && Danbooru.Utility && Danbooru.Utility.notice(message);
};

JSPLib.menu._error = function (message) {
    window.Danbooru && Danbooru.Utility && Danbooru.Utility.error(message);
};
