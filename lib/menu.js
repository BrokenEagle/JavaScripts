...
(function (utility, validate, storage) {
...
menu.version = 11;
menu.program_data_regex = null;
menu.program_data_key = null;
menu.css_debug_url = null;
menu.program_reset_data = {};
...
menu.saveMenuHotkey = function (event) {
    JSPLib._jQuery('#userscript-settings-menu .tab-panel.active-tab .jsplib-settings-buttons [id$="-commit"]').click();
...
menu.installSettingsMenu = function () {
    ...
    if (JSPLib._jQuery("#userscript-settings-menu").length === 0) {
        ...
        let $tab_list = JSPLib._jQuery('.tab-list:has(> .basic-tab)');
        ...
        window_JSPLib.menu_settings_css = utility.setCSSStyle(menu.debug_settings_css || SETTINGS_CSS, 'menu_settings');
        window_JSPLib.menu_color_css = utility.setCSSStyle(menu.debug_color_css || COLOR_CSS, 'menu_color');
        ...
        JSPLib._jQuery(document).on(JSPLib.program.keydown, null, 'alt+s', menu.saveMenuHotkey);
    }
    let $settings_component = JSPLib._jQuery('#userscript-settings-menu');
    ...
    settings_tabs.push(JSPLib._jQuery(`<span class="jsplib-tab" data-name="${JSPLib.program_id}">${JSPLib.program_name}</span>`).get(0));
    settings_panels.push(JSPLib._jQuery(`<div id="${JSPLib.program_id}" class="jsplib-section" data-name="${JSPLib.program_id}"></div>`).get(0));
...
menu.renderMenuFramework = function (menu_config) {
    return MENU_FRAMEWORK_TEMPLATE({
        ...
        control: menu_config.controls.map((control) => menu.renderMenuSection(control, 'controls')).join('\n'),
...
menu.getEnabledDomains = function () {
    ...
    CURRENT_DOMAINS = utility.arrayIntersection(DOMAINS, cookie_domains);
    if (CURRENT_DOMAINS.length === 0) {
        ...
        CURRENT_DOMAINS = utility.dataCopy(DOMAINS);
        ...
        utility.createCookie(JSPLib.program_name, CURRENT_DOMAINS.join(','), null, 'donmai.us');
    }
    return CURRENT_DOMAINS;
};
...
menu.updateUserSettings = function () {
    ...
    JSPLib._jQuery(`#${JSPLib.program_shortcut}-settings [data-setting][data-type]`).each((_, entry) => {
        let $entry = JSPLib._jQuery(entry);
        ...
        switch (type) {
            ...
            case 'selector':
                $entry.find('input').each((_, input) => {
                    ...
                    JSPLib._jQuery(input).checkboxradio('refresh');
                });
                break;
            case 'sortlist':
                JSPLib._jQuery("ul", entry).sortable("destroy");
                JSPLib._jQuery("li", entry).detach().sort((a, b) => {
                    ...
                    JSPLib._jQuery("ul", entry).append(entry);
                });
                JSPLib._jQuery("ul", entry).sortable();
                // falls through
            default:
                // do nothing
        }
...
menu.updateGlobalSettings = function () {
    ...
    JSPLib._jQuery(`#${JSPLib.program_shortcut}-settings .jsplib-selectors[data-setting="domain_selector"] input`).each((_, entry) => {
...
menu.saveUserSettingsClick = function ({local_callback = null} = {}) {
    JSPLib._jQuery(`#${JSPLib.program_shortcut}-commit`).on(JSPLib.program.click, () => {
        ...
        storage.setLocalData(`${JSPLib.program_shortcut}-user-settings`, settings);
        CHANNEL.postMessage({type: 'settings', program_shortcut: JSPLib.program_shortcut, from: JSPLib.UID.value, user_settings: settings});
        if (!invalid_setting) {
        ...
        let $domain_inputs = JSPLib._jQuery(`#${JSPLib.program_shortcut}-settings .${JSPLib.program_shortcut}-global`);
        if ($domain_inputs.length) {
            ...
            if (!utility.arrayEquals(CURRENT_DOMAINS, selected_domains)) {
                ...
                if (enabled_domains.length || disabled_domains.length) {
                    CHANNEL.postMessage({type: 'domain', program_shortcut: JSPLib.program_shortcut, from: JSPLib.UID.value, enabled_domains, disabled_domains, current_domains: selected_domains});
                }
                printer.debuglogLevel("Domains updated:", {new_domains: selected_domains, old_domains: CURRENT_DOMAINS}, JSPLib.debug.VERBOSE);
...
menu.resetUserSettingsClick = function ({delete_keys = [], local_callback = null} = {}) {
    JSPLib._jQuery(`#${JSPLib.program_shortcut}-resetall`).on(JSPLib.program.click, () => {
        let config = menu.settings_config;
        let settings = JSPLib.program_data.user_settings;
        JSPLib.program_data.old_settings = utility.dataCopy(settings);
        if (confirm(`This will reset all of ${JSPLib.program_name}'s settings.\n\nAre you sure?`)) {
            ...
            Object.assign(JSPLib.program_data, utility.dataCopy(menu.program_reset_data), {storage_keys: {local_storage: []}});
            storage.setLocalData(`${JSPLib.program_shortcut}-user-settings`, settings);
            CHANNEL.postMessage({type: 'reset', program_shortcut: JSPLib.program_shortcut, from: JSPLib.UID.value, user_settings: settings});
...
menu.purgeCacheClick = function () {
    JSPLib._jQuery(`#${JSPLib.program_shortcut}-control-purge-cache`).on(JSPLib.program.click, () => {
        if (!PURGE_IS_STARTED && confirm(`This will delete all of ${JSPLib.program_name}'s cached data.\n\nAre you sure?`)) {
            PURGE_IS_STARTED = true;
            storage.purgeCache(menu.program_data_regex).then(() => {
...
menu.cacheInfoClick = function () {
    JSPLib._jQuery(`#${JSPLib.program_shortcut}-control-cache-info`).on(JSPLib.program.click, () => {
        JSPLib.notice.notice("Calculating cache information...");
        storage.programCacheInfo(menu.program_data_regex).then((cache_info) => {
            let template_data = {};
            ...
            JSPLib._jQuery(`#${JSPLib.program_shortcut}-cache-info-table`).html(CACHE_INFO_TEMPLATE(template_data)).show();
        });
    });
};

menu.expandableClick = function () {
    JSPLib._jQuery(`.${JSPLib.program_shortcut}-expandable .jsplib-expandable-button`).on(JSPLib.program.click, (event) => {
...
menu.loadStorageKeys = async function () {
    let program_data_regex = menu.program_data_regex;
...
menu.getCacheDatakey = function () {
    ...
    if (JSPLib.program_data.data_source === "local_storage") {
        ...
        if (typeof menu.program_data_key === "function") {
            data_key = menu.program_data_key(JSPLib.program_data.data_type, JSPLib.program_data.data_value);
        } else if (typeof menu.program_data_key === "object") {
            data_key = menu.program_data_key[JSPLib.program_data.data_type] + '-' + JSPLib.program_data.data_value;
        }
    }
...
menu.saveLocalData = function (key, data, validator, localupdater) {
    if (validator(key, data)) {
        ...
        if (key === `${JSPLib.program_shortcut}-user-settings`) {
            ...
            menu.updateUserSettings();
            CHANNEL.postMessage({type: 'settings', program_shortcut: JSPLib.program_shortcut, from: JSPLib.UID.value, user_settings: data});
...
menu.adjust_data_name = function (disable = true) {
    let name_selector = `#${JSPLib.program_shortcut}-control-data-name`;
    if (disable && DATA_NAME_DISABLED) {
...
menu.dataSourceChange = function () {
    JSPLib._jQuery(`#${JSPLib.program_shortcut}-control-data-source`).on(JSPLib.program.change, () => {
        ...
        menu.adjust_data_name(can_disable);
    });
};

menu.rawDataChange = function () {
    JSPLib._jQuery(`#${JSPLib.program_shortcut}-enable-raw-data`).on(JSPLib.program.change, (event) => {
        DATA_NAME_DISABLED = event.target.checked;
        menu.adjust_data_name();
    });
};

menu.getCacheClick = function (localvalidator) {
    JSPLib._jQuery(`#${JSPLib.program_shortcut}-data-name-get`).on(JSPLib.program.click, () => {
...
menu.saveCacheClick = function (localvalidator, indexvalidator, localupdater) {
    JSPLib._jQuery(`#${JSPLib.program_shortcut}-data-name-save`).on(JSPLib.program.click, () => {
        var data;
        try {
...
menu.deleteCacheClick = function () {
    JSPLib._jQuery(`#${JSPLib.program_shortcut}-data-name-delete`).on(JSPLib.program.click, () => {
        let storage_key = menu.getCacheDatakey();
        if (JSPLib.program_data.data_source === "local_storage") {
...
menu.listCacheClick = function () {
    JSPLib._jQuery(`#${JSPLib.program_shortcut}-data-name-list`).on(JSPLib.program.click, () => {
...
menu.refreshCacheClick = function () {
    JSPLib._jQuery(`#${JSPLib.program_shortcut}-data-name-refresh`).on(JSPLib.program.click, () => {
...
menu.cacheSource = function () {
    return function (_, resp) {
        ...
        if ((JSPLib.program_data.data_source === "indexed_db" && JSPLib.program_data.data_value.length === 0) || !STORAGE_KEYS_LOADED) {
            resp([]);
            return;
        }
        ...
        if (JSPLib.program_data.data_source === 'local_storage') {
            transformed_keys = _keyToNameTransform(transformed_keys, JSPLib.program_shortcut);
        } else if (JSPLib.program_data.data_type !== "custom") {
            let program_keys = transformed_keys.filter((key) => key.match(JSPLib.program_regex));
            let program_names = _keyToNameTransform(program_keys, JSPLib.program_shortcut);
            let cache_keys = utility.arrayDifference(transformed_keys, program_keys);
            let cache_names = cache_keys.map((key) => key.replace(menu.program_data_regex, ''));
...
JSPLib.initializeModule('menu', CONFIGURATION);

})(JSPLib.utility, JSPLib.validate, JSPLib.storage);
