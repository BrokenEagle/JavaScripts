...
// JSPLib
// JSPLib-UI: tabs, checkboxradio, sortable
...
/*Fix because the color is missing from Danbooru*/
...
JSPLib.menu.mainSettingsClick = function (self) {
    if (!JSPLib.utility.isNamespaceBound(this.settings_selector,'click','jsplib')) {
        ...
        JSPLib._jQuery(this.settings_selector).on('click.jsplib',(event)=>{
            JSPLib._jQuery(this.other_selectors).removeClass("active");
            JSPLib._jQuery(event.target).addClass("active");
            JSPLib._jQuery('.edit_user > fieldset').hide();
...
JSPLib.menu.installSettingsMenu = function (self) {
    if (JSPLib._jQuery("#userscript-settings-menu").length === 0) {
        ...
        JSPLib._jQuery('input[name=commit]').before(this.settings_field);
        JSPLib._jQuery("#edit-options").append('| <a href="#userscript-menu">Userscript Menus</a>');
    ...
    JSPLib._jQuery("#userscript-settings-tabs li").sort(function(a, b) {
        try {
...
JSPLib.menu.initializeSettingsMenu = function (RenderSettingsMenu, menu_CSS) {
    ...
    JSPLib.utility.installScript(this.tab_widget_url).done(()=>{
...
JSPLib.menu.renderCacheControls = function () {
    return `
<div id="${this.program_shortcut}-cache-controls" class="jsplib-control-grouping">
...
JSPLib.menu.renderMenuSection = function (value,type) {
    ...
    return `
<div id="${this.program_shortcut}-${value.name}-${type}" class="jsplib-${type}-grouping">
    <div id="${this.program_shortcut}-${value.name}-${type}-message" class="prose">
        <h4>${JSPLib.utility.displayCase(value.name)} ${type}</h4>
        ${message}
    </div>
</div>`;
};
...
JSPLib.menu.renderMenuFramework = function (menu_config) {
    let settings_html = menu_config.settings.map((setting) => JSPLib.menu.renderMenuSection(setting,'settings')).join('\n');
    let control_html = menu_config.controls.map((control) => JSPLib.menu.renderMenuSection(control,'controls')).join('\n');
return `
<div id="${this.program_shortcut}-script-message" class="prose">
    <h2>${this.program_name}</h2>
    <p>Check the forum for the latest on information and updates (<a class="dtext-link dtext-id-link dtext-forum-topic-id-link" href="/forum_topics/${menu_config.topic_id}">topic #${menu_config.topic_id}</a>).</p>
</div>
<div id="${this.program_shortcut}-console" class="jsplib-console">
    <div id="${this.program_shortcut}-settings" class="jsplib-outer-menu">
        ...
        <div id="${this.program_shortcut}-settings-buttons" class="jsplib-settings-buttons">
            <input type="button" id="${this.program_shortcut}-commit" value="Save">
            <input type="button" id="${this.program_shortcut}-resetall" value="Factory Reset">
        </div>
    </div>
...
JSPLib.menu.getCheckboxRadioSelected = function (selector) {
    return JSPLib._jQuery(selector).map((i,input)=>{return (input.checked ? JSPLib._jQuery(input).data('selector') : undefined);}).toArray();
};
...
//For updating domain selectors not changed by the menu, i.e. broadcast
...
JSPLib.menu.saveUserSettingsClick = function (local_callback=null) {
    ...
    let program_data = this.program_data;
    let current_domains = this.getEnabledDomains();
    JSPLib._jQuery(`#${program_shortcut}-commit`).on(this.program_click,(event)=>{
        ...
        JSPLib._jQuery(`#${program_shortcut}-settings .${program_shortcut}-setting[id]`).each((i,entry)=>{
            ...
            if (entry.type === "checkbox" || entry.type === "radio") {
                ...
                if (selector) {
                 ...
                 JSPLib._jQuery(entry).val(settings[setting_name]);
            } else if (entry.type === "hidden") {
        ...
        JSPLib.storage.setStorageData(`${program_shortcut}-user-settings`,settings,localStorage);
        this._channel.postMessage({type: "settings", program_shortcut: program_shortcut, from: JSPLib.UID.value, user_settings: settings});
        if (JSPLib._jQuery(`#${program_shortcut}-settings .${program_shortcut}-global`).length) {
            ...
            if (enabled_domains.length || disabled_domains.length) {
                this._channel.postMessage({type: 'domain', program_shortcut: program_shortcut, from: JSPLib.UID.value, enabled_domains: enabled_domains, disabled_domains: disabled_domains, current_domains: selected_domains});
            }
...
JSPLib.menu.resetUserSettingsClick = function (delete_keys=[],local_callback=null) {
    ...
    let program_name = this.program_name;
    let program_data = this.program_data;
    JSPLib._jQuery(`#${program_shortcut}-resetall`).on(this.program_click,(event)=>{
        let config = this.settings_config;
        let settings = program_data.user_settings;
        program_data.old_settings = JSPLib.utility.dataCopy(settings);
        if (confirm(`This will reset all of ${program_name}'s settings.\n\nAre you sure?`)) {
            ...
            Object.assign(program_data,this.program_reset_data,{storage_keys: {local_storage: []}});
            JSPLib.storage.setStorageData(`${program_shortcut}-user-settings`,settings,localStorage);
            this._channel.postMessage({type: 'reset', program_shortcut: program_shortcut, from: JSPLib.UID.value, user_settings: settings});
...
JSPLib.menu.saveLocalData = function (key,data,validator,localupdater) {
    ...
    if (validator(key,data)) {
        ...
        if (key === `${program_shortcut}-user-settings`) {
            ...
            this._channel.postMessage({type: "settings", program_shortcut: program_shortcut, from: JSPLib.UID.value, user_settings: data});
...
JSPLib.menu.dataSourceChange = function () {
    let program_shortcut = this.program_shortcut;
    JSPLib._jQuery(`#${program_shortcut}-control-data-source`).on(`change.${program_shortcut}`,(event)=>{
...
JSPLib.menu.getCacheClick = function () {
    let program_shortcut = this.program_shortcut;
    let program_data = this.program_data;
    JSPLib._jQuery(`#${program_shortcut}-data-name-get`).on(this.program_click,(event)=>{
...
JSPLib.menu.saveCacheClick = function (localvalidator,indexvalidator,localupdater) {
    let program_shortcut = this.program_shortcut;
    let program_data = this.program_data;
    JSPLib._jQuery(`#${program_shortcut}-data-name-save`).on(this.program_click,(event)=>{
        try {
            var data = JSON.parse(JSPLib._jQuery(`#${program_shortcut}-cache-viewer textarea`).val());
        } catch (error) {
...
JSPLib.menu.deleteCacheClick = function () {
    JSPLib._jQuery(`#${this.program_shortcut}-data-name-delete`).on(this.program_click,(event)=>{
        let storage_key = this.getCacheDatakey();
...
JSPLib.menu.listCacheClick = function () {
    let program_data = this.program_data;
    let program_shortcut = this.program_shortcut;
    JSPLib._jQuery(`#${this.program_shortcut}-data-name-list`).on(this.program_click,(event)=>{
...
JSPLib.menu.refreshCacheClick = function () {
    JSPLib._jQuery(`#${this.program_shortcut}-data-name-refresh`).on(this.program_click,(event)=>{
    ...
    return function (event) {
        ...
        switch (event.data.type) {
            case 'domain':
                ...
                if (event.data.enabled_domains.includes(context._current_subdomain)) {
                    context.program_data.is_enabled = true;
                    if (typeof context.enable_callback === 'function') {
                        context.enable_callback();
                    }
                }
...
JSPLib.menu._configuration = {
    nonenumerable: ['_purge_is_started','_data_name_disabled','_storage_keys_loaded','_storage_keys_promise','_active_script','_channel','_current_subdomain','_current_domains','_isSettingMenu','_broadcastRX','_updateSettingsFromBroadcast','_configuration'],
    nonwritable: ['_channel','_current_subdomain','css_themes_url','settings_css','color_css','settings_field','settings_selector','_configuration']
};
JSPLib.initializeModule('menu');
JSPLib.debug.addModuleLogs('menu',['mainSettingsClick','otherSettingsClicks','installSettingsMenu','getEnabledDomains','loadUserSettings','validateUserSettings']);
