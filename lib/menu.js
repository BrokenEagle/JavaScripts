...
Menu.renderDomainSelectors = function () {
    ...
    let html_items = JSPLib.domains.map((selector) => DOMAIN_ITEM_TEMPLATE({
        shortcut: JSPLib.shortcut,
        selector,
        display: Utility.displayCase(selector),
...
Menu.preloadMenu = function ({menu_func = null, menu_css = null, cache_validation = true} = {}) {
    ...
    jQueryProxy(`#${JSPLib.shortcut}-commit`).on(JSPLib.event.click, () => {
        ...
        if ($domain_inputs.length) {
            ...
            if (!Utility.arrayEquals(CURRENT_DOMAINS, selected_domains)) {
                if (Utility.arrayEquals(JSPLib.domains, selected_domains)) {
...
Menu.getCacheClick = function () {
    jQueryProxy(`#${JSPLib.shortcut}-control-data-name-get`).on(JSPLib.event.click, () => {
        ...
        if (JSPLib.data.data_source === 'local_storage' && JSPLib.data.raw_data) {
            let data = {};
            for (let key in localStorage) {
                ...
                if (!Storage.localSessionValidator(key, temp_data)) {
                    continue;
                }
                data[save_key] = Storage.getLocalData(key);
            }
            p.resolve(data);
        } else if (storage_key === null) {
            Notice.error("No data name entered!");
            return;
        } else if (JSPLib.data.data_source === 'local_storage') {
            p.resolve(Storage.getLocalData(storage_key));
        } else if (JSPLib.data.data_source === 'indexed_db') {
...
Menu.saveCacheClick = function (localupdater) {
    jQueryProxy(`#${JSPLib.shortcut}-control-data-name-save`).on(JSPLib.event.click, () => {
        ...
        if (JSPLib.data.data_source === 'local_storage' && JSPLib.data.raw_data) {
            ...
            if (_saveLocalData(storage_key, data, localupdater)) {
...
function _addStyleSheet(url) {
    let element = JSPLib._document.createElement('link');
    element.rel = 'stylesheet';
    element.type = 'text/css';
    element.href = url;
    JSPLib._document.head.appendChild(element);
}
...
function _getEnabledDomains() {
    ...
    if (!domains) {
        CURRENT_DOMAINS = JSPLib.domains;
    } else if (domains === 'none') {
        CURRENT_DOMAINS = [];
    } else {
        let cookie_domains = domains.split(',');
        CURRENT_DOMAINS = Utility.arrayIntersection(JSPLib.domains, cookie_domains);
        if (CURRENT_DOMAINS.length === 0) {
            printer.logLevel("Invalid cookie found!", Debug.WARNING);
            Utility.eraseCookie(JSPLib.name, 'donmai.us');
            CURRENT_DOMAINS = Utility.deepCopy(JSPLib.domains);
...
async function _loadStorageKeys() {
    ...
    if (Storage.indexed_db_available) {
...
function _getCacheDatakey() {
    ...
    if (!JSPLib.data.data_value) return null;
...
function _cacheSource() {
    return function (_, resp) {
        ...
        if (check_key === null || !STORAGE_KEYS_LOADED) {
            resp([]);
            return;
        }
...
JSPLib.writeOnceReadMany(Menu, 'control_config', {}, 'object');
JSPLib.writeOnceReadMany(Menu, 'settings_callback', () => {}, 'function');
JSPLib.writeOnceReadMany(Menu, 'reset_callback', () => {}, 'function');
JSPLib.writeOnceReadMany(Menu, 'enable_callback', () => {}, 'function');
JSPLib.writeOnceReadMany(Menu, 'disable_callback', () => {}, 'function');
