...
(function (jQuery, utility, validate, storage) {

JSPLib.menu = {};

/****GLOBAL VARIABLES****/

JSPLib.menu.version = 11;
...
const SECTION_TEMPLATE = utility.normalizeHTML({template: true})`
<div
    x-bind:class="{ 'active-tab': active === ${'index'} }"
    class="tab-panel"
>
...
const TEXTINPUT_TEMPLATE = utility.normalizeHTML({template: true})`
<div
    class="${'shortcut'}-textinput jsplib-textinput jsplib-menu-item"
...
const CHECKBOX_TEMPLATE = utility.normalizeHTML({template: true})`
<div
    class="${'shortcut'}-checkbox jsplib-checkbox jsplib-menu-item"
    data-setting="${'setting_name'}"
    data-type="checkbox"
>
    <h4>${'display_name'}</h4>
...
const SORTLIST_TEMPLATE = utility.normalizeHTML({template: true})`
<div
    class="${'shortcut'}-sortlist jsplib-sortlist jsplib-menu-item"
    data-setting="${'setting_name'}"
    data-type="sortlist"
>
    <h4>${'display_name'}</h4>
...
const SORTITEM_TEMPLATE = utility.normalizeHTML({template: true})`
...
const SELECTORS_TEMPLATE = utility.normalizeHTML({template: true})`
<div
    class="${'shortcut'}-selectors jsplib-selectors jsplib-menu-item"
    data-setting="${'setting_name'}"
    data-type="selector"
>
    <h4>${'display_name'}</h4>
...
const SELECTOR_ITEM_TEMPLATE = utility.normalizeHTML({template: true})`
<label for="${'key'}">
    ${'display'}
</label>
<input
...
const KEYSELECT_TEMPLATE = utility.normalizeHTML({template: true})`
<div
    class="${'shortcut'}-options jsplib-options jsplib-menu-item"
    data-setting="${'setting_name'}"
>
    <h4>${'display_name'}</h4>
    <div>
...
const KEYSELECT_ITEM_TEMPLATE = utility.normalizeHTML({template: true})`
<option value="${'option'}" ${'selected'}>
    ${'display'}
</option>`;

const LINKCLICK_TEMPLATE = utility.normalizeHTML({template: true})`
<div class="${'shortcut'}-linkclick jsplib-linkclick jsplib-menu-item">
    <h4>${'display_name'}</h4>
    <div>
        <b>
...
const DOMAIN_TEMPLATE = utility.normalizeHTML({template: true})`
<div
    class="${'shortcut'}-selectors jsplib-selectors jsplib-menu-item"
...
const DOMAIN_ITEM_TEMPLATE = utility.normalizeHTML({template: true})`
<label for="${'key'}">
    ${'display'}
</label>
<input
...
const DATASOURCE_SECTION_TEMPLATE = utility.normalizeHTML({template: true})`
<div
...
const CONTROL_BUTTON_TEMPLATE = utility.normalizeHTML({template: true})`
<input
    id="${'key'}"
...
const SETTING_HINT_TEMPLATE = utility.normalizeHTML({template: true})`
<span class="${'shortcut'}-menu-tooltip jsplib-${'type'}-tooltip">
    ${'text'}
</span>`;

const EXPANDABLE_TEMPLATE = utility.normalizeHTML({template: true})`
...
const CACHE_CONTROLS_TEMPLATE = utility.normalizeHTML({template: true})`
...
const CACHE_INFO_TABLE_TEMPLATE = utility.normalizeHTML({template: true})`
<div
    id="${'shortcut'}-cache-info-table" style="display: none;"
>
</div>`;

const LOCAL_STORAGE_SOURCE_TEMPLATE = utility.normalizeHTML({template: true})`
<input
...
const CACHE_EDITOR_TEMPLATE = utility.normalizeHTML({template: true})`
...
const MENU_SECTION_TEMPLATE = utility.normalizeHTML({template: true})`
...
const MENU_FRAMEWORK_TEMPLATE = utility.normalizeHTML({template: true})`
<div id="${'script'}-script-message" class="prose">
    <h2>${'program_name'}</h2>
    ${'topic'}
    ${'wiki'}
</div>
...
const MENU_TOPIC_MESSAGE_TEMPLATE = utility.normalizeHTML({template: true})`
<p>
...
const DOMAINS = ['danbooru', 'safebooru', 'betabooru', 'kagamihara', 'saitou', 'shima'];
...
JSPLib.menu.saveMenuHotkey = function (event) {
    jQuery('#userscript-settings-menu .tab-panel.active-tab .jsplib-settings-buttons [id$="-commit"]').click();
...
JSPLib.menu.installSettingsMenu = function () {
    ...
    if (jQuery("#userscript-settings-menu").length === 0) {
        ...
        printer.debuglogLevel("Installing menu framework.", JSPLib.debug.DEBUG);
        let $tab_list = jQuery('.tab-list:has(> .basic-tab)');
        ...
        window_JSPLib.menu_settings_css = utility.setCSSStyle(this.debug_settings_css || SETTINGS_CSS, 'menu_settings');
        window_JSPLib.menu_color_css = utility.setCSSStyle(this.debug_color_css || COLOR_CSS, 'menu_color');
        ...
        jQuery(document).on(JSPLib.program.keydown, null, 'alt+s', this.saveMenuHotkey);
    }
    let $settings_component = jQuery('#userscript-settings-menu');
    ...
    let settings_panels = $settings_tab_panels.find('.jsplib-panel').detach().toArray();
    settings_tabs.push(jQuery(`<span class="jsplib-tab" data-name="${JSPLib.program_name}">${JSPLib.program_name}</span>`).get(0));
    settings_panels.push(jQuery(`<div id="${JSPLib.program_id}" class="jsplib-section" data-name="${JSPLib.program_name}"></span>`).get(0));
    settings_tabs.sort((a, b) => a.dataset.name > b.dataset.name);
...
JSPLib.menu.checkDebugCSS = function () {
    if (JSPLib.menu.css_debug_url !== null) {
        if (JSPLib.network && !utility.getPublicData(document.body).jsplibDebugCss) {
            jQuery(document.body).data('jsplib-debug-css', true);
            storage.removeSessionData('jsplib-debug-css');
            return JSPLib.network.getNotify(JSPLib.menu.css_debug_url, {custom_error: "Unable to load debug CSS."}).then((data) => {
        ...
        utility.recheckInterval({
            ...
            exec: () => {
                Object.assign(JSPLib.menu, storage.getSessionData('jsplib-debug-css'));
            },
            always: () => {
                p.resolve(null);
            },
...
JSPLib.menu.renderTextinput = function (setting_name, length = 20, is_control = false) {
    ...
    let submit_control = "";
    if (is_control) {
        submit_control = config[setting_name].buttons.map((button) => this.renderControlButton(setting_key, button, 2)).join("");
    }
    return TEXTINPUT_TEMPLATE({
        shortcut: JSPLib.program_shortcut,
        setting_name,
        display_name,
        value,
        length,
        menu_type,
        submit_control,
        textinput_key: `${JSPLib.program_shortcut}-${menu_type}-${setting_key}`,
        hint: this.renderSettingHint("block", config[setting_name].hint),
    });
};
...
JSPLib.menu.renderCheckbox = function (setting_name, is_control = false) {
    ...
    return CHECKBOX_TEMPLATE({
        ...
        hint: this.renderSettingHint("inline", config[setting_name].hint),
    });
};
...
JSPLib.menu.renderSortlist = function (setting_name) {
    ...
    let html_items = sort_list.map((sortitem) => SORTITEM_TEMPLATE({
        ...
        key: `${JSPLib.program_shortcut}-enable-${setting_key}-${utility.kebabCase(sortitem)}`,
    }));
...
JSPLib.menu.renderInputSelectors = function (setting_name, type, is_control = false, has_submit = false) {
    ...
    let html_items = config[setting_name].allitems.map((selector) => SELECTOR_ITEM_TEMPLATE({
        ...
        key: `${JSPLib.program_shortcut}-select-${setting_key}-${selector}`,
        name: `${JSPLib.program_shortcut}-${menu_type}-${setting_key}`,
    ...
    return SELECTORS_TEMPLATE({
        ...
        html: html_items.join(''),
        hint: this.renderSettingHint("block", config[setting_name].hint),
        submit_control: (is_control && has_submit ? this.renderControlGet(setting_key, 2) : ''),
    });
};

JSPLib.menu.renderDomainSelectors = function () {
    let enabled_selectors = this.getEnabledDomains();
    let html_items = DOMAINS.map((selector) => DOMAIN_ITEM_TEMPLATE({
        shortcut: JSPLib.program_shortcut,
        selector,
        display: utility.displayCase(selector),
        key: `${JSPLib.program_shortcut}-select-domain-${selector}`,
        name: `${JSPLib.program_shortcut}-domain-selector`,
        checked: (enabled_selectors.includes(selector) ? "checked" : ""),
    }));
    return DOMAIN_TEMPLATE({
        shortcut: JSPLib.program_shortcut,
        html: html_items.join(''),
        hint: this.renderSettingHint("block", "Select which domain the script should be active on."),
    });
};
...
JSPLib.menu.renderKeyselect = function (setting_name, is_control = false) {
    ...
    return KEYSELECT_TEMPLATE({
        ...
        display_name,
        menu_type,
        selection_name: `${JSPLib.program_shortcut}-${menu_type}-${setting_key}`,
        html: html_items.join(''),
        hint: this.renderSettingHint("inline", config[setting_name].hint),
    });
};
...
JSPLib.menu.renderControlButton = function (setting_key, button_name) {
    return CONTROL_BUTTON_TEMPLATE({
        shortcut: JSPLib.program_shortcut,
        display: utility.displayCase(button_name),
        key: `${JSPLib.program_shortcut}-${setting_key}-${button_name}`,
    });
};

JSPLib.menu.renderSettingHint = function (type, text) {
...
JSPLib.menu.renderCacheEditor = function (has_cache_data) {
    return CACHE_EDITOR_TEMPLATE({
        shortcut: JSPLib.program_shortcut,
        message: (has_cache_data ? CACHE_EDITOR_MESSAGE_TEMPLATE({shortcut: JSPLib.program_shortcut}) : ""),
    });
};
...
JSPLib.menu.renderMenuFramework = function (menu_config) {
    return MENU_FRAMEWORK_TEMPLATE({
        shortcut: JSPLib.program_shortcut,
        settings: menu_config.settings.map((setting) => this.renderMenuSection(setting, 'settings')).join('\n'),
        control: menu_config.controls.map((control) => this.renderMenuSection(control, 'controls')).join('\n'),
...
JSPLib.menu.engageUI = function ({checkboxradio = false, sortable = false} = {}) {
    if (checkboxradio) {
        jQuery(`.${JSPLib.program_shortcut}-selectors input`).checkboxradio();
    }
    if (sortable) {
        jQuery(`.${JSPLib.program_shortcut}-sortlist ul`).sortable();
    }
...
JSPLib.menu.preloadScript = function (program_value, {render_menu_func = null, run_on_settings = false, default_data = {}, reset_data = {}, initialize_func = null, broadcast_func = null, menu_css = null, program_css = null, light_css = null, dark_css = null, danbooru_userscript = true} = {}) {
    ...
    if (typeof broadcast_func == 'function') {
        program_value.channel = utility.createBroadcastChannel(JSPLib.program_name, broadcast_func);
    }
    if (program_css) {
        utility.setCSSStyle(program_css, 'program');
    }
    if (light_css) {
        utility.setCSSStyle(utility.renderColorScheme(light_css, 'light'), 'light');
    }
    if (dark_css) {
        utility.setCSSStyle(utility.renderColorScheme(dark_css, 'dark'), 'dark');
    }
...
JSPLib.menu.getEnabledDomains = function () {
    ...
    let domains = utility.readCookie(JSPLib.program_name);
    if (!domains) {
        return DOMAINS;
    }
    if (domains === 'none') {
    ...
    this._current_domains = utility.arrayIntersection(DOMAINS, cookie_domains);
    if (this._current_domains.length === 0) {
        printer.debuglogLevel("Invalid cookie found!", JSPLib.debug.WARNING);
        utility.eraseCookie(JSPLib.program_name, 'donmai.us');
        this._current_domains = utility.dataCopy(DOMAINS);
        ...
        utility.createCookie(JSPLib.program_name, this._current_domains.join(','), null, 'donmai.us');
    }
    return this._current_domains;
};
...
JSPLib.menu.loadUserSettings = function () {
    ...
    let settings = storage.getLocalData(`${JSPLib.program_shortcut}-user-settings`, {default_val: {}});
    let dirty = false;
    ...
    if (dirty) {
        ...
        storage.setLocalData(`${JSPLib.program_shortcut}-user-settings`, settings);
    }
...
JSPLib.menu.validateCheckboxRadio = function (data, type, items, {min_length = 0} = {}) {
...
JSPLib.menu.validateNumber = function (data, {integer = false, min = -Infinity, max = Infinity} = {}) {
...
JSPLib.menu.updateUserSettings = function () {
    ...
    jQuery(`#${JSPLib.program_shortcut}-settings [data-setting][data-type]`).each((_, entry) => {
        let $entry = jQuery(entry);
        ...
        switch (type) {
            ...
            case 'selector':
                $entry.find('input').each((_, input) => {
                    ...
                    jQuery(input).checkboxradio('refresh');
                });
                break;
            case 'sortlist':
                jQuery("ul", entry).sortable("destroy");
                jQuery("li", entry).detach().sort((a, b) => {
                    ...
                    jQuery("ul", entry).append(entry);
                });
                jQuery("ul", entry).sortable();
                // falls through
            default:
                // do nothing
        }
...
JSPLib.menu.updateGlobalSettings = function () {
    ...
    jQuery(`#${JSPLib.program_shortcut}-settings .jsplib-selectors[data-setting="domain_selector"] input`).each((_, entry) => {
...
JSPLib.menu.saveUserSettingsClick = function ({local_callback = null} = {}) {
    jQuery(`#${JSPLib.program_shortcut}-commit`).on(JSPLib.program.click, () => {
        const printer = JSPLib.debug.getFunctionPrint('menu.saveUserSettingsClick');
        let config = this.settings_config;
        let settings = JSPLib.program_data.user_settings;
        JSPLib.program_data.old_settings = utility.dataCopy(settings);
        ...
        printer.debuglogLevel("Settings updated:", {new_settings: settings, old_settings: JSPLib.program_data.old_settings, invalid_setting}, JSPLib.debug.VERBOSE);
        storage.setLocalData(`${JSPLib.program_shortcut}-user-settings`, settings);
        ...
        let $domain_inputs = jQuery(`#${JSPLib.program_shortcut}-settings .${JSPLib.program_shortcut}-global`);
        if ($domain_inputs.length) {
            ...
            if (!utility.arrayEquals(this._current_domains, selected_domains)) {
                if (utility.arrayEquals(DOMAINS, selected_domains)) {
                    //Don't bother storing a cookie if all of the domains are active
                    utility.eraseCookie(JSPLib.program_name, 'donmai.us');
                } else if (selected_domains.length === 0) {
                    utility.createCookie(JSPLib.program_name, 'none', 365, 'donmai.us');
                } else {
                    utility.createCookie(JSPLib.program_name, selected_domains.join(','), 365, 'donmai.us');
                }
                let enabled_domains = utility.arrayDifference(selected_domains, this._current_domains);
                let disabled_domains = utility.arrayDifference(this._current_domains, selected_domains);
                ...
                printer.debuglogLevel("Domains updated:", {new_domains: selected_domains, old_domains: this._current_domains}, JSPLib.debug.VERBOSE);
...
JSPLib.menu.resetUserSettingsClick = function ({delete_keys = [], local_callback = null} = {}) {
    jQuery(`#${JSPLib.program_shortcut}-resetall`).on(JSPLib.program.click, () => {
        let config = this.settings_config;
        let settings = JSPLib.program_data.user_settings;
        JSPLib.program_data.old_settings = utility.dataCopy(settings);
        if (confirm(`This will reset all of ${JSPLib.program_name}'s settings.\n\nAre you sure?`)) {
            ...
            Object.assign(JSPLib.program_data, utility.dataCopy(this.program_reset_data), {storage_keys: {local_storage: []}});
            storage.setLocalData(`${JSPLib.program_shortcut}-user-settings`, settings);
...
JSPLib.menu.purgeCacheClick = function () {
    jQuery(`#${JSPLib.program_shortcut}-control-purge-cache`).on(JSPLib.program.click, () => {
        if (!this._purge_is_started && confirm(`This will delete all of ${JSPLib.program_name}'s cached data.\n\nAre you sure?`)) {
            this._purge_is_started = true;
            storage.purgeCache(this.program_data_regex).then(() => {
...
JSPLib.menu.cacheInfoClick = function () {
    jQuery(`#${JSPLib.program_shortcut}-control-cache-info`).on(JSPLib.program.click, () => {
        JSPLib.notice.notice("Calculating cache information...");
        storage.programCacheInfo(this.program_data_regex).then((cache_info) => {
            let template_data = {};
            ...
            jQuery(`#${JSPLib.program_shortcut}-cache-info-table`).html(CACHE_INFO_TEMPLATE(template_data)).show();
        });
    });
};

JSPLib.menu.expandableClick = function () {
    jQuery(`.${JSPLib.program_shortcut}-expandable .jsplib-expandable-button`).on(JSPLib.program.click, (event) => {
        let $container = jQuery(event.target).closest('.jsplib-expandable');
...
JSPLib.menu.loadStorageKeys = async function () {
    ...
    if (program_data_regex) {
        this._storage_keys_promise = storage.danboorustorage.keys();
        ...
        let program_keys = cache_keys.filter((key) => key.match(JSPLib.program_regex));
        storage_keys.indexed_db = utility.concat(program_keys, storage_keys.indexed_db);
    } else {
        this._storage_keys_loaded = true;
    }
    let keys = Object.keys(localStorage);
    storage_keys.local_storage = keys.filter((key) => key.match(JSPLib.program_regex));
};

JSPLib.menu.getCacheDatakey = function () {
    JSPLib.program_data.data_source = jQuery(`#${JSPLib.program_shortcut}-control-data-source`).val();
    JSPLib.program_data.data_type = jQuery(`#${JSPLib.program_shortcut}-control-data-type`).val();
    let data_key = JSPLib.program_data.data_value = jQuery(`#${JSPLib.program_shortcut}-control-data-name`).val().trim();
    if (JSPLib.program_data.data_source === "local_storage") {
        JSPLib.program_data.raw_data = jQuery(`#${JSPLib.program_shortcut}-enable-raw-data`).prop('checked');
...
JSPLib.menu.dataSourceChange = function () {
    jQuery(`#${JSPLib.program_shortcut}-control-data-source`).on(JSPLib.program.change, () => {
        let data_source = jQuery(`#${JSPLib.program_shortcut}-control-data-source`).val();
        jQuery(`.${JSPLib.program_shortcut}-section-data-source`).hide();
        let shown_key = `#${JSPLib.program_shortcut}-section-${utility.kebabCase(data_source)}`;
...
JSPLib.menu.rawDataChange = function () {
    jQuery(`#${JSPLib.program_shortcut}-enable-raw-data`).on(JSPLib.program.change, (event) => {
...
JSPLib.menu.getCacheClick = function (localvalidator) {
    jQuery(`#${JSPLib.program_shortcut}-data-name-get`).on(JSPLib.program.click, () => {
        let storage_key = this.getCacheDatakey();
        if (JSPLib.program_data.data_source === "local_storage") {
            let data = {};
            if (JSPLib.program_data.raw_data) {
                for (let key in localStorage) {
                    let match = key.match(JSPLib.program_regex);
                    if (!match) continue;
            ...
            jQuery(`#${JSPLib.program_shortcut}-cache-viewer textarea`).val(JSON.stringify(data, null, 2));
        } else {
...
JSPLib.menu.saveCacheClick = function (localvalidator, indexvalidator, localupdater) {
    jQuery(`#${JSPLib.program_shortcut}-data-name-save`).on(JSPLib.program.click, () => {
        var data;
        try {
            data = JSON.parse(jQuery(`#${JSPLib.program_shortcut}-cache-viewer textarea`).val());
        } catch (error) {
            JSPLib.notice.error("Invalid JSON data! Unable to save.");
            return;
        }
        validate.dom_output = `#${JSPLib.program_shortcut}-cache-editor-errors`;
        let storage_key = this.getCacheDatakey();
        if (JSPLib.program_data.data_source === "local_storage") {
            if (JSPLib.program_data.raw_data) {
                let error_messages = [];
                let $cache_errors = jQuery(`#${JSPLib.program_shortcut}-cache-editor-errors`);
                for (let key in data) {
...
JSPLib.menu.deleteCacheClick = function () {
    jQuery(`#${JSPLib.program_shortcut}-data-name-delete`).on(JSPLib.program.click, () => {
        let storage_key = this.getCacheDatakey();
        if (JSPLib.program_data.data_source === "local_storage") {
...
JSPLib.menu.listCacheClick = function () {
    jQuery(`#${JSPLib.program_shortcut}-data-name-list`).on(JSPLib.program.click, () => {
        ...
        this._storage_keys_promise.then(() => {
            jQuery(`#${JSPLib.program_shortcut}-cache-viewer textarea`).val(JSON.stringify(JSPLib.program_data.storage_keys[JSPLib.program_data.data_source], null, 2));
        });
    });
};

JSPLib.menu.refreshCacheClick = function () {
    jQuery(`#${JSPLib.program_shortcut}-data-name-refresh`).on(JSPLib.program.click, () => {
...
JSPLib.menu.cacheAutocomplete = function () {
    let $control_data = jQuery(`#${JSPLib.program_shortcut}-control-data-name`);
    ...
    autocomplete._renderItem = function (menu, item) {
        return jQuery("<li>").append(jQuery("<div>").text(item.label)).appendTo(menu);
    };
};
...
JSPLib.menu._broadcastRX = function () {
    ...
    return function (event) {
        ...
        JSPLib.debug.debuglogLevel(`menu.broadcastRX[${iteration++}]`, `(${event.data.type}):`, event.data, JSPLib.debug.INFO);
...
JSPLib.menu._broadcastReset = function (event, menu) {
    Object.assign(menu.program_data, utility.dataCopy(menu.program_reset_data));
...
Object.defineProperties(JSPLib.menu, {
    program_selector: {get: () => '#' + JSPLib.program_id}
});

JSPLib.menu._configuration = {
    nonenumerable: [],
    nonwritable: ['_channel', '_current_subdomain']
};
JSPLib.initializeModule('menu');

})(JSPLib._jQuery, JSPLib.utility, JSPLib.validate, JSPLib.storage);
