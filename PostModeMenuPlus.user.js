...
function DestroyTooltip(post_id) {
    let image = $(`#post_${post_id} .post-preview-image`).get(0);
...
function CommentaryDialog(post_ids) {
    ...
    if (!PMM.commentary_dialog) {
        ...
        PMM.commentary_dialog.dialog(Object.assign({
            ...
            open: OpenCommentaryDialog,
            close: CloseCommentaryDialog,
        }, COMMENTARY_DIALOG_SETTINGS));
        PMM.commentary_dialog.find('#pmm-fetch-post button').on(PROGRAM_CLICK, FetchCommentary);
...
function FetchCommentary(event) {
    ...
    if (JSPLib.validate.validateID(post_id)) {
        ...
        JSPLib.danbooru.submitRequest(`posts/${post_id}/artist_commentary`, {only: ARTIST_COMMENTARY_FIELDS}).then((artist_commentary) => {
...
function OpenCommentaryDialog() {
    let post_ids = PMM.commentary_post_ids;
    let $fetch_input = $('#pmm-fetch-post input');
    if (post_ids.length === 1) {
        ...
        let $fetch_button = $('#pmm-fetch-post button');
        ...
        RelationshipCheck(post_ids).then((post_id) => {
            if (post_id !== null) {
                ...
                UpdateCommentaryTags($(`#post_${post_id}`).data('tags'));
            } else {
                $fetch_input.val("");
            }
...
function CloseCommentaryDialog() {
    $('#close-notice-link').get(0).click();
