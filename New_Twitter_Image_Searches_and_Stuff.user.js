...
(({Debug, Notice, Utility, Storage, Validate, Template, Concurrency, Statistics, Network, Danbooru, Saucenao, Load, Menu}) => {
...
#ntisas-tweet-stats-table td.ntisas-metric-selected {
...
#new-twitter-image-searches-and-stuff input[type=button],
#new-twitter-image-searches-and-stuff input[type=file]::file-selector-button {
    border: 1px solid;
    border-radius: 3px;
    padding: 0.25em 1em;
    cursor: pointer;
...
.ntisas-horizontal-rule {
    border-top-color: %TEXT50%;
    margin: 10px;
}
.ntisas-vertical-rule {
    display: inline-block;
    border-left-color: %TEXT50%;
    height: 200px;
}
/**Side menu**/
#ntisas-side-menu {
...
.ntisas-media-menu .ntisas-media-images .ntisas-media-image,
.ntisas-media-menu .ntisas-media-images .ntisas-media-video {
    box-shadow: 0 0 0 4px %BACKGROUNDCOLOR%, 0 0 0 6px %TEXT10%;
}
/**Qtips**/
.qtiptisas.qtiptisas-twitter {
    color: %TEXTCOLOR%;
    background-color: %BACKGROUNDCOLOR%;
    border: 1px solid %TEXT50%;
}
/**Post result popup**/
.ntisas-post-result .ntisas-image-container {
...
.ui-dialog.ui-dialog-twitter.ntisas-dialog .ui-checkboxradio-radio-label.ui-state-active .ui-icon-background {
    border: 4px solid %BASEDARK%;
}
...
const NTISAS_MENU = Template.normalizeHTML()`
        ...
        <div id="ntisas-menu-info" data-selector="info" style="display:none">
            <table>
                ...
                <tr>
                    ...
                    <td><span id="ntisas-page-max">...</span></td>
                    <td>(%MAXIDHELP%)</td>
                </tr>
                <tr>
                    <td><span>Page min:</span></td>
                    <td><span id="ntisas-page-min">...</span></td>
                    <td>(%MINIDHELP%)</td>
                </tr>
                <tr>
                    <td><span>Time range:</span></td>
                    <td><span id="ntisas-time-range">...</span></td>
                    <td>(%TIMERANGEHELP%)</td>
                </tr>
                <tr>
...
const TWEET_STATISTICS = Template.normalizeHTML()`
<table>
    <tbody>
    <tr>
...
const MEDIA_STATISTICS = Template.normalizeHTML()`
<table>
    <tbody>
    <tr>
        <th data-key="total"><a class="ntisas-metric">Total</a></th>
...
const NTISAS_TWEET_MENU_HELP = Template.normalizeHTML()`
...
const NTISAS_TWEET_MENU = Template.normalizeHTML()`
...
const NO_MATCH_LINK = Template.normalizeHTML()`
 <a
...
const SELECTION_CONTROLS = Template.normalizeHTML()`
...
const SIMILAR_SOURCE_HTML = Template.normalizeHTML()`
...
const VIEW_HIGHLIGHTS_HTML = Template.normalizeHTML()`
...
const PROFILE_TIMELINE_HTML = Template.normalizeHTML()`
...
const LOCKPAGE_HTML = Template.normalizeHTML()`
...
const STATUS_MARKER = Template.normalizeHTML()`
...
const CURRENT_RECORDS_CONFIRM = Template.trim`
...
const CONFIRM_DELETE_PROMPT = Template.trim`
...
const VIEWCOUNT_RECENT_DURATION = Utility.one_minute * 5;
const POST_VERSIONS_CALLBACK = Utility.one_second * 5;
const PAGE_REFRESH_TIMEOUT = Utility.one_second * 5;
const SAUCE_EXPIRES = Utility.one_hour;
const MIN_POST_EXPIRES = Utility.one_day;
const MAX_POST_EXPIRES = Utility.one_month;
const USER_EXPIRES = Utility.one_month;
const VIEW_EXPIRES = Utility.one_month * 2;
const TWEET_EXPIRES = Utility.one_week;
const TWUSER_EXPIRES = Utility.one_day;
const TWLIST_EXPIRES = Utility.one_day;
const LENGTH_RECHECK_EXPIRES = Utility.one_hour;
const USER_PROFILE_RECHECK_EXPIRES = Utility.one_month;
const BADVER_RECHECK_EXPIRES = Utility.one_day;
const PRUNE_RECHECK_EXPIRES = Utility.one_hour * 6;
const PROFILE_VIEWS_CALLBACK = Utility.one_second * 10;
const USER_ID_CALLBACK = Utility.one_second * 5;
const DATABASE_BATCHSAVE_INTERVAL = Utility.one_second * 15;
...
const TWEET_REGEX = Template.verboseRegex()`${TWITTER_HOST}/(?:i/web|${TWITTER_ACCOUNT})/status/(${TWITTER_ID})(?:/photo/\d+|/video/\d+)?${QUERY_END}`;
const SOURCE_TWITTER_REGEX = Template.verboseRegex()`^source:${TWEET_REGEX.source.slice(1)}`;
const TWEET_REGEXG = new RegExp(TWEET_REGEX.source, 'g');
const SOURCE_TWITTER_REGEXG = new RegExp(SOURCE_TWITTER_REGEX.source, 'g');

const BANNER_REGEX = Template.verboseRegex()`https://pbs\.twimg\.com/profile_banners/(\d+)/\d+/`;

const TWITTER_IMAGE1 = Template.verboseRegex('i')`
${TWIMG_HOST_RG}
/
...
const TWITTER_IMAGE2 = Template.verboseRegex('i')`
${TWIMG_HOST_RG}
/
...
const TWITTER_IMAGE3 = Template.verboseRegex('i')`
${TWIMG_HOST_RG}
/
(?<path>media|tweet_video_thumb)
/
...
const TWITTER_IMAGE4 = Template.verboseRegex('i')`
${TWIMG_HOST_RG}
/
(?<path>(?:ext_tw_video_thumb|amplify_video_thumb)/\d+(?:/\w+)?/img)
/
...
const UNHANDLED_IMAGES = [
    Template.verboseRegex()`^https://pbs\.twimg\.com/profile_images/`,
    Template.verboseRegex()`^https://[^.]+\.twimg\.com/emoji/`,
    Template.verboseRegex()`^https://pbs\.twimg\.com/ad_img/`,
    Template.verboseRegex()`^https://abs\.twimg\.com/hashflags/`,
    Template.verboseRegex()`^https://pbs\.twimg\.com/card_img/`,
    Template.verboseRegex()`^https://abs\.twimg\.com/sticky/`,
    Template.verboseRegex()`^https://abs\.twimg\.com/responsive-web/`,
];

const MAIN_PAGE_REGEX = Template.verboseRegex('i')`
${TWITTER_HOST}
/
(?!search|home)
...
const MEDIA_PAGE_REGEX = Template.verboseRegex('i')`
${TWITTER_HOST}
/
(?<account>${TWITTER_ACCOUNT})
/media
...
const LIKES_PAGE_REGEX = Template.verboseRegex('i')`
${TWITTER_HOST}
/
(?<account>${TWITTER_ACCOUNT})
/likes
...
const REPLIES_PAGE_REGEX = Template.verboseRegex('i')`
${TWITTER_HOST}
/
(?<account>${TWITTER_ACCOUNT})
/with_replies
...
const SEARCH_PAGE_REGEX = Template.verboseRegex('i')`
${TWITTER_HOST}
/
search\?
(?<query>.*?\bq=.+)
...
const QUOTES_PAGE_REGEX = Template.verboseRegex('i')`
${TWITTER_HOST}
/
(?<account>${TWITTER_ACCOUNT})
/status/
...
const TWEET_PAGE_REGEX = Template.verboseRegex('i')`
${TWITTER_HOST}
/
(?<account>${TWITTER_ACCOUNT})
/status/
...
const WEB_TWEET_PAGE_REGEX = Template.verboseRegex('i')`
${TWITTER_HOST}
/
i/web
/status/
...
const PHOTO_PAGE_REGEX = Template.verboseRegex('i')`
${TWITTER_HOST}
/
(?<account>${TWITTER_ACCOUNT})
/status/
...
const HASHTAG_PAGE_REGEX = Template.verboseRegex('i')`
${TWITTER_HOST}
/
hashtag/
(?<hashtag>[^?]+)
...
const LIST_PAGE_REGEX = Template.verboseRegex('i')`
${TWITTER_HOST}
/
(?<account>${TWITTER_ACCOUNT})
/lists/
...
const HOME_PAGE_REGEX = Template.verboseRegex('i')`
${TWITTER_HOST}
/
home
${QUERY_END}
...
const EVENTS_PAGE_REGEX = Template.verboseRegex('i')`
${TWITTER_HOST}
/
i/events/
(?<id>${TWITTER_ID})
...
const TOPICS_PAGE_REGEX = Template.verboseRegex('i')`
${TWITTER_HOST}
/
i/topics/
(?<id>${TWITTER_ID})
...
const DISPLAY_PAGE_REGEX = Template.verboseRegex('i')`
${TWITTER_HOST}
/
settings/display
${QUERY_END}
...
const STORAGE_DATABASES = {
    ...
    twitter: Storage.twitterstorage,
};
...
const SHOWN_MENU_PAGES = Utility.concat(STREAMING_PAGES, ['tweet', 'web_tweet']);
...
const TWEET_CONSTRAINTS = {
    ...
    value: {
        partial_image: Validate.stringonly_constraints,
        partial_video: Validate.stringnull_constraints,
        partial_sample: Validate.stringnull_constraints,
    },
};
...
const COLOR_CONSTRAINTS = {
    base_color: Validate.array_constraints({is: 3}),
    text_color: Validate.array_constraints({is: 3}),
    background_color: Validate.array_constraints({is: 3})
};
...
function ValidateEntry(key, entry) {
    ...
    if (key.match(/^twuser-/)) {
        return Validate.validateHashEntries(key, entry, TWUSER_CONSTRAINTS);
    }
    if (key.match(/^tweet-/)) {
    ...
    if (key === 'ntisas-available-sauce') {
        return Validate.validateHashEntries(key, entry, SAUCE_CONSTRAINTS);
    }
...
function ValidateTypeEntry(key, entry, type, constraints) {
    if (!Validate.validateHashEntries(key, entry, constraints.entry)) {
        return false;
    }
    if (type === 'hash') {
        return Validate.validateHashEntries(key + '.value', entry.value, constraints.value);
    }
    return Validate.validateHashArrayEntries(key + '.value', entry.value, constraints.value)
}
...
function GetDateString(timestamp) {
    ...
    return `${time_obj.getFullYear()}${Utility.padNumber(time_obj.getMonth() + 1, 2)}${Utility.padNumber(time_obj.getDate(), 2)}`;
}

function GetTimeString(timestamp) {
    let time_obj = new Date(timestamp);
    return `${Utility.padNumber(time_obj.getHours(), 2)}${Utility.padNumber(time_obj.getMinutes(), 2)}`;
}
...
function RemoveHashKeyValue(hash, key, value) {
    if ((key in hash) && hash[key].includes(value)) {
        hash[key] = Utility.arrayDifference(hash[key], [value]);
...
function ChangeColorArray(array, amount) {
    return array.map((val) => Utility.clamp(Number(val) + amount, 0, 255).toString());
}
...
async function GetNormalImageURL(image_info) {
    let printer = Debug.getFunctionPrint('GetNormalImageURL');
    if (!(image_info.key in NTISAS.known_extensions)) {
        if (image_info.ext === 'webp') {
            printer.log("Checking webp image for extension:", image_info);
...
async function AddViewCount(tweet_id) {
    if (NTISAS.count_views && !NTISAS.recorded_views.includes(tweet_id)) {
        ...
        let data_expires = Utility.getExpires(VIEW_EXPIRES);
...
function PromptSavePostIDs(tweet_id, message, initial_post_ids) {
    let printer = Debug.getFunctionPrint('PromptSavePostIDs');
...
function UpdateProfileCallback() {
    ...
    if (NTISAS.display_user_id) {
        $('.ntisas-profile-user-id').html(Utility.sprintf(PROFILE_USER_ID, ''));
    }
    if (NTISAS.display_profile_views) {
        $('.ntisas-profile-user-view').html(Utility.sprintf(PROFILE_USER_VIEW, ''));
        $('.ntisas-profile-stream-view').html(Utility.sprintf(PROFILE_STREAM_VIEW, ''));
    }
    NTISAS.update_profile = Utility.recheckInterval({
        ...
        interval: TIMER_POLL_INTERVAL,
        duration: PROFILE_VIEWS_CALLBACK,
    });
}

function UnhideTweets() {
    let printer = Debug.getFunctionPrint('UnhideTweets');
...
function UpdateSideMenu(page_type, update_visibility) {
    let menu_shown = Storage.getLocalData('ntisas-side-menu', {default_val: true});
...
function UpdateSideMenuSelection() {
    let selected_menu = Storage.getLocalData('ntisas-side-selection');
...
function UpdateSimilarControls() {
    NTISAS.similar_source = Storage.getLocalData('ntisas-similar-source', {default_val: 'danbooru'});
...
function UpdateConfirmUploadControls() {
    NTISAS.confirm_upload = Storage.getLocalData('ntisas-confirm-upload', {default_val: false});
...
function UpdateConfirmDownloadControls() {
    NTISAS.confirm_download = Storage.getLocalData('ntisas-confirm-download', {default_val: false});
...
function UpdateViewHighlightControls() {
    let show_highlights = Storage.getLocalData('ntisas-view-highlights', {default_val: true});
...
function UpdateViewCountControls() {
    let count_views = NTISAS.count_views = Storage.getLocalData('ntisas-view-counts', {default_val: true});
...
function UpdateThumbnails(container, all_posts) {
    all_posts.forEach(async (post) => {
        let blob = await Network.getData(post.thumbnail);
...
function PageNavigation(pagetype) {
    let printer = Debug.getFunctionPrint('PageNavigation');
    //Use all non-URL matching groups as a page key to detect page changes
    let page_key = NTISAS.page_match.groups ?
        Utility.arrayUnique(
            Object.values(NTISAS.page_match.groups).filter((val) => (Utility.isString(val) && !val.startsWith('https:')))
        ).join(',') :
        "";
    ...
    var params;
    let account = NTISAS.page_match.groups?.account.toLowerCase();
    ...
    switch (NTISAS.page) {
        ...
        case 'likes':
        case 'replies':
            printer.log(`User timeline [${NTISAS.page}]:`, account);
        ...
        case 'topics':
        case 'events':
            printer.log(`Stream timeline [${NTISAS.page}]:`, page_id ?? "n/a");
            NTISAS.account = NTISAS.user_id = undefined;
            if (NTISAS.page === 'list') {
                NTISAS.list_id = page_id;
            }
            break;
        case 'hashtag':
            printer.log("Hashtag timeline:", NTISAS.page_match.groups.hashtag);
            NTISAS.account = NTISAS.user_id = undefined;
            NTISAS.hashtag_search = location.search;
            NTISAS.params = Utility.parseParams(location.search);
            break;
        case 'search':
            printer.log("Search timeline:", NTISAS.page_match.groups.query);
            params = Utility.parseParams(NTISAS.page_match.groups.query);
        ...
        case 'tweet':
        case 'web_tweet':
            printer.log("Tweet ID:", page_id);
            NTISAS.screen_name = account;
            NTISAS.tweet_id = page_id;
            NTISAS.account = NTISAS.user_id = undefined;
            break;
        case 'display':
            printer.log("Twitter display settings");
            return;
        default:
            //Do nothing
    }
    ...
    if (IsPageType(STREAMING_PAGES) || IsTweetPage()) {
        if ($('#ntisas-side-menu').length === 0) {
            ...
            InitializeTotalRecords().then((total) => {
                ...
                $('#ntisas-total-records').on(JSPLib.event.click, QueryTotalRecords);
            });
        }
        //Bind events for creation/rebind
        if (!Utility.isNamespaceBound({root: '#ntisas-open-settings', eventtype: 'click', namespace: PROGRAM_SHORTCUT})) {
            $('#ntisas-menu-selection a').on(JSPLib.event.click, SideMenuSelection);
            $('#ntisas-current-records').on(JSPLib.event.click, CurrentRecords);
            $('#ntisas-similar-toggle a').on(JSPLib.event.click, ToggleSimilarSource);
            $('#ntisas-confirm-upload-toggle a').on(JSPLib.event.click, ToggleConfirmUpload);
            $('#ntisas-confirm-download-toggle a').on(JSPLib.event.click, ToggleConfirmDownload);
            $('#ntisas-view-highlights-toggle a').on(JSPLib.event.click, ToggleViewHighlights);
            $('#ntisas-view-counts-toggle a').on(JSPLib.event.click, ToggleViewCounts);
            $('#ntisas-open-settings').on(JSPLib.event.click, OpenSettingsMenu);
            //These will only get bound here on a rebind
            $('#ntisas-total-records').on(JSPLib.event.click, QueryTotalRecords);
            $('#ntisas-error-messages').on(JSPLib.event.click, ErrorMessages);
            $('#ntisas-lockpage-toggle a').on(JSPLib.event.click, ToggleLock);
        }
...
function SetCheckPostvers() {
    if (Concurrency.checkTimeout('ntisas-timeout', GetPostVersionsExpiration()) || WasOverflow()) {
        clearTimeout(NTISAS.postvers_timeout);
        NTISAS.postvers_timeout = setTimeout(() => {
            if (NTISAS.database_info && Concurrency.reserveSemaphore('postvers')) {
                CheckPostvers();
            }
...
async function GetAllCurrentRecords() {
    ...
    while (true) {
        ...
        clearTimeout(NTISAS.postvers_timeout);
        if (Concurrency.reserveSemaphore('postvers')) {
            Notice.notice(`Querying Danbooru...[${i}]`, false, false);
            await CheckPostvers();
        } else {
            Notice.notice(`Waiting on other tasks to finish...[${i}]`, false, false);
            await Utility.sleep(POST_VERSIONS_CALLBACK);
        }
        i++;
    }
}
...
async function CheckPostvers() {
    let printer = Debug.getFunctionPrint('CheckPostvers');
    ...
    if (post_versions.length === QUERY_BATCH_SIZE) {
        ...
        printer.log("No overflow:", post_versions.length, QUERY_BATCH_SIZE);
        Storage.setLocalData('ntisas-overflow', false);
    }
    if (post_versions.length) {
        let [add_entries, rem_entries] = ProcessPostvers(post_versions);
        printer.log("Process:", add_entries, rem_entries);
        ...
        if (Utility.isID(lastid)) {
            ...
            Storage.setLocalData('ntisas-recent-timestamp', most_recent_timestamp);
            InitializeCurrentRecords();
            NTISAS.channel.postMessage({type: 'currentrecords'});
        }
    }
    Concurrency.setRecheckTimeout('ntisas-timeout', GetPostVersionsExpiration());
    Concurrency.freeSemaphore('postvers');
}

async function CheckServerBadTweets() {
    let printer = Debug.getFunctionPrint('CheckServerBadTweets');
    if (NTISAS.database_info && Concurrency.checkTimeout('ntisas-badver-recheck', BADVER_RECHECK_EXPIRES) && Concurrency.reserveSemaphore('badvers')) {
        ...
        if (post_versions.length === QUERY_BATCH_SIZE) {
            ...
            printer.log("No overflow:", post_versions.length, QUERY_BATCH_SIZE);
            Concurrency.setRecheckTimeout('ntisas-badver-recheck', BADVER_RECHECK_EXPIRES);
        }
        if (post_versions.length) {
            let [add_entries, rem_entries] = ProcessPostvers(post_versions);
            printer.log("Process:", add_entries, rem_entries);
        ...
        Concurrency.freeSemaphore('badvers');
    }
}
...
function GetPostVersionsLastID(type) {
    ...
    Storage.invalidateLocalData(storage_key);
    let postver_lastid = Storage.checkLocalData(storage_key, {default_val: NTISAS.database_info?.post_version});
    ...
    if (postver_lastid !== max_postver_lastid) {
        Storage.setLocalData(`ntisas-${type}-lastid`, max_postver_lastid);
    }
    return max_postver_lastid;
}

function SavePostvers(add_entries, rem_entries) {
    let printer = Debug.getFunctionPrint('SavePostvers');
    let combined_keys = Utility.arrayIntersection(Object.keys(add_entries), Object.keys(rem_entries));
    combined_keys.forEach((tweet_id) => {
        ...
        Storage.retrieveData(tweet_key, {bypass_cache: true, database: Storage.twitterstorage}).then((data) => {
            ...
            if (data === null || Utility.arraySymmetricDifference(post_ids, data)) {
                printer.logLevel("Tweet adds/rems - saving:", tweet_key, post_ids, Debug.DEBUG);
    ...
    let single_adds = Utility.arrayDifference(Object.keys(add_entries), combined_keys);
    single_adds.forEach((tweet_id) => {
        ...
        Storage.retrieveData(tweet_key, {bypass_cache: true, database: Storage.twitterstorage}).then((data) => {
            ...
            if (data === null || post_ids.length > data.length) {
                printer.log("Tweet adds - saving:", tweet_key, post_ids);
                SaveData(tweet_key, post_ids, 'twitter');
                UpdatePostIDsLink(tweet_id, post_ids);
                NTISAS.channel.postMessage({type: 'postlink', tweet_id, post_ids});
            }
    ...
    let single_rems = Utility.arrayDifference(Object.keys(rem_entries), combined_keys);
    single_rems.forEach((tweet_id) => {
        ...
        Storage.retrieveData(tweet_key, {bypass_cache: true, database: Storage.twitterstorage}).then((data) => {
            ...
            if (post_ids.length) {
                printer.log("Tweet removes - saving:", tweet_key, post_ids);
                SaveData(tweet_key, post_ids, 'twitter');
            } else {
                printer.log("Tweet removes - deleting:", tweet_key);
...
function ProcessPostvers(postvers) {
    let printer = Debug.getFunctionPrint('ProcessPostvers');
    ...
    postvers.forEach((postver) => {
        if (postver.source_changed) {
            if (postver.version === 1) {
                let tweet_id = Utility.findAll(postver.source, TWEET_REGEXG)[1];
                if (tweet_id) {
                    add_entries[tweet_id] ??= [];
                    add_entries[tweet_id] = Utility.arrayUnion(add_entries[tweet_id], [postver.post_id]);
                } else {
                    printer.warn("Unfound new post:", postver.source, postver);
                }
            } else {
                let tweet_id = {};
                let twitter_add = postver.added_tags.find((tag) => SOURCE_TWITTER_REGEX.test(tag)) ?? "";
                tweet_id.add = Utility.findAll(twitter_add, SOURCE_TWITTER_REGEXG)[1];
                let twitter_rem = postver.removed_tags.find((tag) => SOURCE_TWITTER_REGEX.test(tag)) ?? "";
                tweet_id.rem = Utility.findAll(twitter_rem, SOURCE_TWITTER_REGEXG)[1];
                ...
                if (tweet_id.add && tweet_id.rem) {
                    if (tweet_id.add === tweet_id.rem) {
                        ...
                        link_modify = true;
                        link_modifies++;
                        printer.log("Link modify detected", twitter_rem, "->", twitter_add);
                    } else {
                        account_swaps++;
                        printer.log("ID swap detected", tweet_id.rem, "->", tweet_id.add);
                    }
                }
                if (tweet_id.add) {
                    add_entries[tweet_id.add] = add_entries[tweet_id.add] || [];
                    add_entries[tweet_id.add] = Utility.arrayUnion(add_entries[tweet_id.add], [postver.post_id]);
                    if (RemoveHashKeyValue(rem_entries, tweet_id.add[0], postver.post_id)) {
                        printer.log("Source delete reversal detected", tweet_id.add);
                    }
                }
                if (tweet_id.rem) {
                    rem_entries[tweet_id.rem] = rem_entries[tweet_id.rem] || [];
                    Utility.arrayUnion(rem_entries[tweet_id.rem], [postver.post_id]);
                    if (RemoveHashKeyValue(add_entries, tweet_id.rem, postver.post_id)) {
                        printer.log("Source add reversal detected", tweet_id.rem);
                    }
                }
                if (!tweet_id.add && !tweet_id.rem && !link_modify) {
                    printer.warn("Unfound edit post:", postver.added_tags, postver.removed_tags, twitter_add, twitter_rem, postver);
                }
            }
        }
        if (postver.added_tags.includes('bad_twitter_id') || postver.removed_tags.includes('bad_twitter_id')) {
            ...
            if (tweet_id) {
                if (postver.removed_tags.includes('bad_twitter_id')) {
                    printer.log("Activated tweet:", tweet_id);
                    add_entries[tweet_id] ??= [];
                    add_entries[tweet_id] = Utility.arrayUnion(add_entries[tweet_id], [postver.post_id]);
                    reversed_posts++;
                    if (RemoveHashKeyValue(rem_entries, tweet_id, postver.post_id)) {
                        printer.log("Tweet remove reversal detected", tweet_id);
                    }
                } else if (postver.added_tags.includes('bad_twitter_id')) {
                    rem_entries[tweet_id] ??= [];
                    rem_entries[tweet_id] = Utility.arrayUnion(rem_entries[tweet_id], [postver.post_id]);
                    inactive_posts++;
                    if (RemoveHashKeyValue(add_entries, tweet_id, postver.post_id)) {
                        printer.log("Tweet add reversal detected", tweet_id);
                    }
                }
            }
        }
    ...
    if (account_swaps > 0) {
        printer.log("Account swaps detected:", account_swaps);
    }
    if (link_modifies > 0) {
        printer.log("Link modifies detected:", link_modifies);
    }
    if (inactive_posts > 0) {
        printer.log("Inactive tweets detected:", inactive_posts);
    }
    if (reversed_posts > 0) {
        printer.log("Activated tweets detected:", reversed_posts);
    }
...
function ReadFileAsync(fileselector, is_json) {
    let printer = Debug.getFunctionPrint('ReadFileAsync');
    return new Promise((resolve, reject) => {
        ...
        reader.onloadend = function(event) {
            if (event.target.readyState === FileReader.DONE) {
                printer.log("File loaded:", file.size);
                let data = event.target.result;
                if (is_json) {
                    try {
                        data = JSON.parse(data);
                    } catch (e) {
                        Notice.error("Error: File is not JSON!");
                        reject();
                    }
                }
                resolve(data);
...
function RenderCurrentRecords() {
    var record_html = "";
    let timestamp = Storage.checkLocalData('ntisas-recent-timestamp');
    if (timestamp) {
        let timestring = new Date(timestamp).toLocaleString();
        let timeagostring = ((Date.now() - timestamp) < GetPostVersionsExpiration() * 2 ? "Up to date" : Utility.timeAgo(timestamp));
...
function RenderSimilarContainer(image_result, index) {
    ...
    if (image_result.results.length > 0) {
        image_result.results.forEach((iqdb_result) => {
            ...
            let is_user_upload = post.uploaderid === NTISAS.user_data.id;
            let similarity_score = Utility.setPrecision(iqdb_result.score, 2);
...
function RenderColorStyle(css) {
    return Utility.regexReplace(css, {
        BASECOLOR: NTISAS.color.base_100,
        BASE50: NTISAS.color.base_50,
        BASE20: NTISAS.color.base_20,
        BASEDARK: NTISAS.color.base_dark,
        TEXTCOLOR: NTISAS.color.text_100,
        TEXT50: NTISAS.color.text_50,
        TEXT20: NTISAS.color.text_20,
        TEXT10: NTISAS.color.text_10,
        BACKGROUNDCOLOR: NTISAS.color.background,
        TEXTINPUT: NTISAS.color.textinput,
    });
}
...
async function InitializeTotalRecords(manual = false) {
    if (manual || Concurrency.checkTimeout('ntisas-length-recheck', LENGTH_RECHECK_EXPIRES)) {
        let database_length = await Storage.twitterstorage.length();
        Storage.setLocalData('ntisas-database-length', database_length);
        Concurrency.setRecheckTimeout('ntisas-length-recheck', LENGTH_RECHECK_EXPIRES);
    }
    return Storage.getLocalData('ntisas-database-length', {default_val: 0});
}
...
async function InitializelUserProfileData() {
    ...
    if (!NTISAS.user_data || Concurrency.checkTimeout('ntisas-user-profile-recheck', USER_PROFILE_RECHECK_EXPIRES)) {
        ...
        Storage.setLocalData('ntisas-user-data', NTISAS.user_data);
        Concurrency.setRecheckTimeout('ntisas-user-profile-recheck', USER_PROFILE_RECHECK_EXPIRES);
    }
}

function InitializeColorScheme() {
    if (!Utility.hasStyle('color')) {
        let color_data = Storage.checkLocalData('ntisas-color-style');
        if (color_data) {
            NTISAS.colors = color_data;
            SetColorValues()
            let color_style = RenderColorStyle(COLOR_CSS);
            Utility.setCSSStyle(color_style, 'color');
        }
    }
}
...
function InitializeSideMenu() {
    ...
    let selected_menu = Storage.checkLocalData('ntisas-side-selection', {default_val: 'info'});
...
function InitializeDatabaseLink() {
    ...
    Storage.retrieveData('ntisas-database-info', {bypass_cache: true, database: Storage.twitterstorage}).then((database_info) => {
        var database_html, database_help;
        if (Utility.isHash(database_info)) {
...
function InitializeMediaTweet(tweet_id, post_ids, tweet_dict_promise) {
    ...
    if (post_ids !== null) {
        $media_icon.append(Utility.sprintf(MEDIA_RESULTS_ICON, 'ntisas-media-match', String(post_ids.length)));
    } else {
        $media_icon.append(Utility.sprintf(MEDIA_RESULTS_ICON, 'ntisas-media-nomatch', '·'));
    }
    if ($tweet.hasClass('ntisas-multi-media')) {
        tweet_dict_promise.then((tweet_dict) => {
            $media_icon.append(Utility.sprintf(MEDIA_COUNTER_ICON, String(tweet_dict[tweet_id].length)));
        });
    } else {
        if (GetSingleImageInfo(tweet_id).length) {
            $media_icon.append(Utility.sprintf(MEDIA_COUNTER_ICON, '·'));
        } else {
            let image = $tweet.find(`img`).get(0);
            if (image) {
                image.onload = function () {
                    Debug.log("Image loaded late:", tweet_id, image, image.src);
                    $tweet.find('.ntisas-media-counter').text('·');
                };
                image.onerror = function () {
                    Debug.warn("Image failed to load:", tweet_id, image);
                    $tweet.find('.ntisas-media-counter').text('X');
                };
            }
            $media_icon.append(Utility.sprintf(MEDIA_COUNTER_ICON, '0'));
        }
    }
    $media_icon.on(JSPLib.event.click, OpenMediaTweetMenu);
    if (NTISAS.display_tweet_views) {
        GetData('view-' + tweet_id, 'danbooru').then((views) => {
            if (views && views.value.count > 0) {
                let timeagostring = ((Date.now() - views.value.viewed) < VIEWCOUNT_RECENT_DURATION ? "recently" : Utility.timeAgo(views.value.viewed));
        ...
        content: {
            text: (_, qtip) => {
                if (!qtip.tooltip[0].hasAttribute(PROGRAM_SHORTCUT)) {
                    if (delayfunc) {
                        var timer;
                        timer = Utility.initializeInterval(() => {
...
function InitializeTwitterImage(article, image_urls, preview_dimensions) {
    ...
    image_promise.then(({size, width, height}) => {
        ...
        let size_text = (size > 0 ? Utility.readableBytes(size) : 'Unavailable');
...
function InitializePostsContainer(all_posts, image_urls) {
    ...
    image_urls.forEach((image_url) => {
        Network.getDataSize(image_url + ':orig').then((size) => {
...
function InitializeConfirmDialog(tweet_id, similar_results, posts) {
    ...
    let image_urls = Utility.getObjectAttributes(similar_results, 'image_url');
...
function InitializeUploadDialog(tweet_id, screen_name, image_urls, videos) {
    ...
    if (!NTISAS.upload_dialog[tweet_id]) {
        ...
        $dialog.find('.ntisas-image-container a').on(JSPLib.event.click, PopupTweetLargeImage);
        let dialog_settings = Object.assign({}, GENERAL_DIALOG_SETTINGS, {
            title: "Upload Menu",
            buttons: {
...
async function InitializeViewCount(tweet) {
    let printer = Debug.getFunctionPrint('ToggleImageSize');
    let tweet_id = String($(tweet).data('tweet-id'));
    let views = await GetData('view-' + tweet_id, 'danbooru');
    if (views && views.value.count > 0) {
        let timeagostring = ((Date.now() - views.value.viewed) < VIEWCOUNT_RECENT_DURATION ? "recently" : Utility.timeAgo(views.value.viewed));
...
function InitializeProfileViewCount(views, data_key, selector, format) {
    ...
    if (views && views.value.count > 0) {
        ...
        view_time = ((Date.now() - views.value.viewed) < VIEWCOUNT_RECENT_DURATION ? "recently" : Utility.timeAgo(views.value.viewed));
    }
    let display_text = Utility.sprintf(format, view_time);
    ...
    if (!NTISAS.recorded_views.includes(data_key)) {
        ...
        let data_expires = Utility.getExpires(Utility.one_year);
...
function InitializeProfileTimeline() {
    ...
    if ($info.length === 1) {
        ...
        printer.warn("Unable to find profile attachment point!", $info);
    }
}

function InitializeImageTweets($image_tweets) {
    let printer = Debug.getFunctionPrint('InitializeImageTweets');
    var $process_tweets;
    ...
    $process_tweets.each((_, tweet) => {
        ...
        Utility.recheckInterval({
            ...
            interval: TWITTER_DELAY,
            duration: Utility.one_second * 10,
        });
    });
    Promise.all(promise_array).then(() => {
        Debug.timeEnd(timername);
    });
}

function InitializeTweetStats(filter1, filter2) {
    const getStatAverage = (obj, type) => {
        let type_array = Utility.getObjectAttributes(obj, type);
        let adjusted_array = Statistics.removeOutliers(type_array, 1);
        let average = Statistics.average(adjusted_array);
        return Utility.setPrecision(average, 2);
    };
    ...
    let table_html = Utility.regexReplace(TWEET_STATISTICS, {
    ...
    let selected_metrics = Utility.arrayUnique([filter1, filter2]);
    if (selected_metrics.length === 2 && selected_metrics.includes('total')) {
        selected_metrics.splice(selected_metrics.indexOf('total'), 1);
    }
    $('#ntisas-tweet-stats-table td').removeClass('ntisas-metric-selected');
    selected_metrics.forEach((metric) => {
        $(`#ntisas-tweet-stats-table td[data-key=${metric}]`).addClass('ntisas-metric-selected');
    });
    return true;
}
...
function IntervalStorageHandler() {
    let printer = Debug.getFunctionPrint('IntervalStorageHandler');
...
async function TimelineHandler() {
    ...
    let accounts = Utility.arrayUnique(TIMELINE_REQUESTS.map((request) => request.account));
    for (let i = 0; i < accounts.length; i++) {
        ...
        while (lowest_requested_id < lowest_available_id) {
            ...
            if (Object.keys(tweet_data.tweets).length === 0) {
                Debug.warn("TimelineHandler - No found tweets:", tweet_data);
                return;
            }
    ...
    if (TIMELINE_REQUESTS.length && TimelineHandler.errors < 3) {
        setTimeout(() => {TimelineHandler();}, Utility.one_second);
    }
}
...
async function CheckURL(tweet_id, screen_name) {
    ...
    if (twitter_posts.length > 0) {
        ...
        post_ids = Utility.arrayUnique(Utility.getObjectAttributes(twitter_posts, 'id'));
    }
    return post_ids;
}
...
async function CheckSimilar(tweet_id, image_urls) {
    ...
    if (similar_results.length > 0) {
        let all_results = Utility.getObjectAttributes(similar_results, 'results').flat();
        let all_posts = Utility.getObjectAttributes(all_results, 'post');
        let unique_posts = RemoveDuplicates(all_posts, 'id');
        let post_ids = Utility.getObjectAttributes(unique_posts, 'id');
...
async function CheckIQDB(image_urls) {
    ...
    let similar_results = [];
    if (flat_data.length) {
        let post_data = Utility.getObjectAttributes(flat_data, 'post');
...
async function CheckSauce(image_urls) {
    ...
    if (filtered_data.length) {
        let danbooru_ids = Utility.arrayUnique(Utility.getNestedObjectAttributes(filtered_data, ['data', 'danbooru_id']));
    ...
    let combined_headers = Utility.getObjectAttributes(good_data, 'header');
    ...
    Storage.saveData('ntisas-available-sauce', {value: sauce_remaining, expires: Utility.getExpires(SAUCE_EXPIRES)});
    return similar_results;
}
...
function DownloadMediaFile(file_url, tweet_id, screen_name, order) {
    let printer = Debug.getFunctionPrint('DownloadMediaFile');
    const mime_types = {
        jpg: 'image/jpeg',
        png: 'image/png',
        mp4: 'video/mp4',
    ...
    let download_name = Utility.regexReplace(NTISAS.filename_prefix_format, {
        DATE: date_string,
        TIME: time_string,
        TWEETID: tweet_id,
    ...
    if (mime_type) {
        printer.log("Saving", file_url, "as", download_name);
        Network.getData(file_url, {ajax_options: {timeout: 0}}).then(
            //Success
            (blob) => {
        ...
        Notice.error("Unknown mime type for extension:", extension);
    }
    return p.promise;
}
...
function TwitterGraphQLRequest(endpoint, variables, features) {
    ...
    let csrf_token = Utility.readCookie('ct0');
    const ajax_options = {
        processData: false,
        beforeSend (request) {
    ...
    return Network.get(`https://${location.host}/i/api/graphql/${endpoint}?${url_addons}`, {ajax_options});
}
...
async function GetMediaTweetsGQL(tweet_ids, account) {
    ...
    tweet_ids.forEach((tweet_id, i) => {
        if (Utility.isHash(network_data[i])) {
            network_data[i].id_str = tweet_id;
        }
    });
    return network_data.filter((data) => Utility.isHash(data));
}
    ...
    if (!GetTweetData.memoized[tweet_id]) {
        ...
        GetData(data_key, 'danbooru').then(async (storage_data) => {
            if (!storage_data) {
                ...
                if (Utility.isHash(network_data)) {
...
function GetTweetsData(tweet_ids, account) {
    ...
    const promise_dict = {};
    let unchecked_tweet_ids = Utility.arrayDifference(tweet_ids, Object.keys(GetTweetsData.memoized));
    if (unchecked_tweet_ids.length) {
        unchecked_tweet_ids.forEach((tweet_id) => {
            promise_dict[tweet_id] = Utility.createPromise();
        ...
        Promise.all(promise_array).then(async (storage_data) => {
            ...
            let query_tweet_ids = Utility.arrayDifference(unchecked_tweet_ids, storage_tweet_ids);
            if (query_tweet_ids.length) {
                ...
                if (Utility.isArray(network_data)) {
                    ...
                    let missing_tweet_ids = Utility.arrayDifference(query_tweet_ids, network_tweet_ids);
                    expires = Utility.getExpires(Utility.one_day);
        ...
        if (typeof data[i] === "object" && data[i] !== null) {
            if (('legacy' in data[i]) && ('rest_id' in data[i])) {
                ...
                if ((i === "tweet" || i === "user")) {
                    ...
                    type = data[i].__typename.toLowerCase();
                }
                if (type) {
                    let key = type + 's';
                    let item = data[i].legacy;
                    item.id_str = data[i].rest_id;
                    if (type === 'user' && 'core' in data[i]) {
                        Utility.assignObjects(item, data[i].core);
                    }
...
function HandleTwitterErrorResponse(response, notice) {
    var message, html;
    if (Utility.isHash(response.responseJSON)) {
...
function SaveTweetData(tweet_id, post_ids) {
    ...
    let merge_ids = Utility.arrayUnion(existing_ids, post_ids);
...
function GetSessionTweetData(tweet_id) {
    return Storage.getIndexedSessionData('tweet-' + tweet_id, {default_val: [], database: STORAGE_DATABASES.twitter});
}

function WasOverflow() {
    return Storage.checkLocalData('ntisas-overflow', {default_val: false});
}

//Database functions

async function SaveDatabase(database, counter_selector) {
    let printer = Debug.getFunctionPrint('SaveDatabase');
    var database_keys = Object.keys(database);
    let batches = Math.floor(database_keys.length / 2000);
    printer.log("Database size:", database_keys.length);
    var payload = {};
    var i;
    for (i = 0; i < database_keys.length; i++) {
        ...
        if (i !== 0 && (i % 2000 === 0)) {
            ...
            printer.log("Saving batch #", batches);
            Debug.time('database-save-' + batches);
            await Storage.twitterstorage.setItems(payload);
            Debug.timeEnd('database-save-' + batches);
...
async function InitializeSaveDatabaseBatches() {
    let import_pending = Storage.getLocalData('ntisas-import-pending');
    if (!import_pending) return;
    let keylist = await Storage.batchstorage.keys();
    ...
    if (batch_keys.length > 0) {
        ...
        Storage.setLocalData('ntisas-database-batchindex', Math.min(...batch_indices));
        Storage.setLocalData('ntisas-database-numbatches', Math.max(...batch_indices) + 1);
        NTISAS.database_interval = setInterval(SaveDatabaseBatch, DATABASE_BATCHSAVE_INTERVAL);
    } else {
        Storage.setLocalData('ntisas-import-pending', false);
    }
}
...
function SaveDatabaseBatch() {
    ...
    if (Utility.isInteger(num_batches) && Utility.isInteger(batch_index) && num_batches > batch_index) {
        ...
        Storage.setLocalData('ntisas-database-batchindex', batch_index + 1);
        Storage.batchstorage.getItem('database-batch-' + batch_index).then((database_batch) => {
            if (Utility.isHash(database_batch)) {
                (async () => {
                    Debug.time('SaveDatabaseBatch-' + display_index);
                    ...
                    await Storage.twitterstorage.setItems(payload);
                    await Storage.batchstorage.removeItem('database-batch-' + batch_index);
                    Debug.timeEnd('SaveDatabaseBatch-' + display_index);
                })();
            } else {
        ...
        Storage.removeLocalData('ntisas-database-numbatches');
        Storage.removeLocalData('ntisas-database-batchindex');
...
async function GetSavePackage(export_types) {
    ...
    if (export_types.includes('program_data')) {
        Object.keys(localStorage).forEach((key) => {
            if (key.match(/^ntisas-/)) {
                let temp_data = Storage.checkLocalData(key);
                if (temp_data !== null) {
    ...
    if (export_types.includes('tweet_database')) {
        ...
        await Storage.twitterstorage.iterate((value, key, i) => {
            let match = key.match(/^tweet-(\d+)$/);
            if (match) {
...
function CheckViews(entries, observer) {
    ...
    entries.forEach((entry) => {
        if (entry.isIntersecting) {
            ...
            printer.logLevel("Viewable tweet:", tweet_id, entry, Debug.DEBUG);
            AddViewCount(tweet_id);
            $tweet.attr('viewed', 'true');
...
function SaveMenuPosition() {
    ...
    if (left !== "" && top !== "") {
        Storage.setLocalData('ntisas-menu-position', {left, top});
...
function ImageEnter(event) {
    ...
    let style = Utility.sprintf(IMAGE_NUMBER_CSS, image_num, margin_top, margin_left);
    Utility.setCSSStyle(style, 'image_num');
}
...
function ToggleImageSize(event) {
    ...
    try {
        ...
        if (image_info) {
            ...
            printer.warn("No match!", image_url);
        }
    } catch (e) {
        Notice.error("ToggleImage: Error");
...
function ToggleSideMenu(event) {
    let menu_shown = Storage.getLocalData('ntisas-side-menu', {default_val: true});
    Storage.setLocalData('ntisas-side-menu', !menu_shown);
...
function SideMenuSelection(event) {
    ...
    Storage.setLocalData('ntisas-side-selection', selected_menu);
...
function ToggleSimilarSource() {
    ...
    Storage.setLocalData('ntisas-similar-source', similar_source);
    UpdateSimilarControls();
    NTISAS.channel.postMessage({type: 'similar_source'});
}

function ToggleConfirmUpload() {
    Storage.setLocalData('ntisas-confirm-upload', !NTISAS.confirm_upload);
    UpdateConfirmUploadControls();
    NTISAS.channel.postMessage({type: 'confirm_upload'});
}

function ToggleConfirmDownload() {
    Storage.setLocalData('ntisas-confirm-download', !NTISAS.confirm_download);
    UpdateConfirmDownloadControls();
...
function ToggleViewHighlights() {
    let show_highlights = Storage.getLocalData('ntisas-view-highlights', {default_val: true});
    Storage.setLocalData('ntisas-view-highlights', !show_highlights);
    UpdateViewHighlights();
    UpdateViewHighlightControls();
...
function ToggleViewCounts() {
    let count_views = Storage.getLocalData('ntisas-view-counts', {default_val: true});
    Storage.setLocalData('ntisas-view-counts', !count_views);
    UpdateViewCountControls();
...
function CurrentRecords() {
    if (!CurrentRecords.is_running) {
        ...
        if (WasOverflow()) {
            if (Concurrency.reserveSemaphore('records')) {
                if (confirm(CURRENT_RECORDS_CONFIRM)) {
                    GetAllCurrentRecords();
                }
                Concurrency.freeSemaphore('records');
            } else {
                Notice.error("Getting current records in another tab!");
            }
        } else {
            Notice.notice("Already up to date!");
        }
...
function QueryTotalRecords() {
    InitializeTotalRecords(true).then((length) => {
        ...
        Notice.notice("Finished updating record count!");
    });
}
...
function OpenMediaTweetMenu(event) {
    ...
    if (!NTISAS.media_dialog[tweet_id]) {
        ...
        var tweet_data;
        if ($tweet.hasClass('ntisas-multi-media')) {
            let session_data = Storage.getIndexedSessionData('tweet-' + tweet_id, {default_val: []});
        ...
        video_promise.then((videos) => {
            ...
            $dialog.find('.ntisas-media-image').each((_, entry) => {
                $(entry).on(JSPLib.event.click, PopupMediaTweetImage);
            });
            $dialog.find('.ntisas-media-video').each((_, entry) => {
                $(entry).on(JSPLib.event.click, PopupMediaTweetVideo);
            });
            let encoded_url = encodeURIComponent(`https://twitter.com/${screen_name}/status/${tweet_id}`);
...
function PopupMediaTweetVideo(event) {
    ...
    let tweet_data = Storage.getIndexedSessionData('tweet-' + tweet_id, {default_val: []});
    let image_data = tweet_data.value[order];
    if (Utility.isHash(image_data)) {
...
function SearchMenu(event) {
    ...
    GetImageLinks($tweet[0]).then((image_urls) => {
        if (image_urls.length > 0) {
            ...
            Notice.error(`Unable to find any images to query for tweet #${tweet_id}.`);
        }
    });
}
...
function SearchSubmit(event) {
    ...
    if (image_urls.length > 0) {
        ...
        Notice.notice("Must select at least one image to query.");
    }
    CloseDialog(event);
}
...
function UploadMenu(event) {
    ...
    GetMediaLinksData($tweet).then(({image_urls, videos}) => {
        if (image_urls.length > 1) {
            ...
            Notice.error(`Unable to find any images to upload for tweet #${tweet_id}.`);
        }
    });
    event.preventDefault();
}
...
function DownloadMenu(event) {
    ...
    GetMediaLinksData($tweet).then(({image_urls, videos}) => {
        if (image_urls.length > 1) {
            ...
            Notice.error(`Unable to find any images to download for tweet #${tweet_id}.`);
        }
    });
    event.preventDefault();
}
...
function SelectMetric(event) {
    ...
    if (InitializeTweetStats(filter1, filter2)) {
        ...
        Notice.notice("Must select category combinations with at least one tweet!");
    }
}
...
function ImportData() {
    let printer = Debug.getFunctionPrint('ImportData');
        ...
        ReadFileAsync('#ntisas-import-file', true).then(
            ...
            async (import_package) => {
                ...
                if ('database_info' in import_package) {
                    ...
                    printer.log("Database info:", import_package.database_info);
                    await Storage.saveData('ntisas-database-info', import_package.database_info, {database: Storage.twitterstorage});
                ...
                if ('database_batches' in import_package) {
                    ...
                    for (let i = 0; i < import_package.database_batches.length; i++) {
                        printer.log(`"Saving batch #${i + 1} of ${import_package.database_batches.length}`);
                        $('#ntisas-import-counter').text(import_package.database_batches.length - i);
                        await Storage.batchstorage.setItem('database-batch-' + i, import_package.database_batches[i]);
                        await Utility.sleep(100);
                    }
                    Storage.setLocalData('ntisas-import-pending', true);
                    $('#ntisas-import-counter').text(0);
                }
                if ('tweet_database' in import_package) {
                    printer.log("Database length:", Object.keys(import_package.tweet_database).length);
                    await SaveDatabase(import_package.tweet_database, '#ntisas-import-counter');
                }
                if (errors) {
                    Notice.error("Error importing some data!");
                } else {
                    Notice.notice("Finished importing data.");
                }
                NTISAS.import_is_running = false;
                await Utility.sleep(Utility.one_second * 2);
...
function MarkupMediaType(tweet) {
    let printer = Debug.getFunctionPrint('MarkupMediaType');
    if ($('.ntisas-tweet-media [src*="/card_img/"]', tweet).length) {
        ...
        printer.log("Delaying media check for", $(tweet).data('tweet-id'));
...
function MarkupStreamTweet(tweet) {
    let printer = Debug.getFunctionPrint('MarkupStreamTweet');
    try {
        ...
        let is_pinned = Boolean(tweet_status.innerText.match(/^Pinned/));
        $(tweet).attr('data-is-pinned', is_pinned);
        ...
        saved_index = index;
        if (sub_body.children[index].querySelector('button') && sub_body.children[index].innerText === "") {
            index++;
        }
...
function MarkupMainTweet(tweet) {
    let printer = Debug.getFunctionPrint('MarkupMainTweet');
    try {
...
function CheckHiddenMedia(tweet) {
    ...
    if ($('.ntisas-tweet-media', tweet).text().match(/The following media includes potentially sensitive content|The Tweet author flagged this Tweet as showing sensitive content|The post author flagged this post as showing sensitive content/)) {
        ...
        $('.ntisas-tweet-media [role=button]', tweet).one(JSPLib.event.click, () => {
...
function RegularCheck() {
    ...
    if (!IsPageType(['tweet', 'web_tweet', 'photo', 'other'])) {
        CollectTweetStats();
    }
...
function ProcessTweetImage(obj, image_info, unprocessed_tweets) {
    let printer = Debug.getFunctionPrint('ProcessTweetImage');
    let $obj = $(obj);
    if (image_info) {
        ...
        Debug.execute(() => {
            if (Utility.isBoolean(image_info)) {
                ...
                printer.warn("Unhandled image", obj.src, $obj.closest('[ntisas-tweet]').data('tweet-id'));
            }
        });
    }
}
...
function ProcessTweetImages() {
    let printer = Debug.getFunctionPrint('ProcessTweetImages');
    ...
    if ($unprocessed_images.length) {
        printer.log("Images found:", $unprocessed_images.length);
    }
    ...
    if ($unprocessed_videos.length) {
        printer.log("Videos found:", $unprocessed_videos.length);
    }
    ...
    if (total_unprocessed > 0) {
        printer.log("Tweets updated:", total_unprocessed);
    }
}

function ProcessNewTweets() {
    let printer = Debug.getFunctionPrint('ProcessNewTweets');
    ...
    NTISAS.uniqueid = Utility.getUniqueID();
    printer.log(NTISAS.uniqueid);
    let main_tweets = [];
    $tweets.each((_, entry) => {
        $(entry).attr({
            viewed: false,
    ...
    printer.log(`[${NTISAS.uniqueid}]`, "New:", $tweets.length, "Image:", $image_tweets.length);
...
function ProcessMediaTweets() {
    ...
    $tweet_list_items.each((_, tweet) => {
        ...
        if (!Utility.isString(tweet_id)) return;
    ...
    UpdateViewHighlights();
    let table_html = Utility.regexReplace(MEDIA_STATISTICS, {
    ...
    UpdateRanges([...timeline_tweets.total]);
}
...
function AdjustColorScheme() {
    ...
    Storage.localforage.getItem('device:rweb.settings').then((settings) => {
        if (Utility.isHash(settings) && $home_button.length && document.body.style['background-color']) {
            ...
            if (!NTISAS.colors || Utility.objectReduce(new_colors, compareColors, false)) {
                NTISAS.colors = new_colors;
                SetColorValues();
                let color_style = RenderColorStyle(COLOR_CSS);
                Utility.setCSSStyle(color_style, 'color');
                Storage.setLocalData('ntisas-color-style', NTISAS.colors);
            }
        }
    });
}
...
function BroadcastNTISAS(ev) {
    let printer = Debug.getFunctionPrint('BroadcastNTISAS');
    printer.log(`(${ev.data.type}):`, ev.data);
    switch (ev.data.type) {
        case 'postlink':
            if (ev.data.post_ids.length) {
                Storage.setSessionData('tweet-' + ev.data.tweet_id, ev.data.post_ids);
            } else {
                Storage.removeSessionData('tweet-' + ev.data.tweet_id);
            }
        ...
        case 'currentrecords':
            ...
            break;
        case 'sidemenu_ui':
            Storage.invalidateLocalData('ntisas-side-menu');
            UpdateSideMenu(NTISAS.page, true);
            break;
        case 'sidemenu_selection':
            Storage.invalidateLocalData('ntisas-side-selection');
            UpdateSideMenuSelection();
            break;
        ...
        case 'view_highlights':
            Storage.invalidateLocalData('ntisas-view-highlights');
            UpdateViewHighlights();
            break;
        case 'similar_source':
            Storage.invalidateLocalData('ntisas-similar-source');
            UpdateSimilarControls();
            break;
        case 'confirm_upload':
            Storage.invalidateLocalData('ntisas-confirm-upload');
            UpdateConfirmUploadControls();
            break;
        case 'confirm_download':
            Storage.invalidateLocalData('ntisas-confirm-download');
            UpdateConfirmDownloadControls();
            // falls through
        default:
            //do nothing
...
function InitializeChangedSettings() {
    ...
    $processed_tweets.each((_, tweet) => {
        ...
        let post_ids = GetSessionTweetData(tweet_id);
        if ($post_link.length && Menu.hasSettingChanged('advanced_tooltips_enabled')) {
            if (NTISAS.advanced_tooltips_enabled) {
                InitializePostIDsLink(tweet_id, tweet, post_ids);
            } else {
        ...
        if (Menu.hasSettingChanged('display_tweet_views')) {
            if (NTISAS.display_tweet_views) {
        ...
        if ($post_link.length && ((post_ids.length > 1 && Menu.hasSettingChanged('custom_order_enabled')))) {
            $post_link.qtiptisas('destroy', true);
            InitializePostIDsLink(tweet_id, tweet, post_ids);
        }
    });
    let called_profile_callback = false;
    if (Menu.hasSettingChanged('display_user_id')) {
    ...
    if (Menu.hasSettingChanged('auto_unhide_tweets_enabled') && NTISAS.auto_unhide_tweets_enabled) {
        UnhideTweets();
    }
    if (Menu.hasSettingChanged('display_profile_views') && IsPageType(['main', 'media', 'likes', 'replies'])) {
    ...
    if (Menu.hasSettingChanged('self_tweet_highlights') && IsPageType(['main', 'likes', 'replies'])) {
        if (NTISAS.self_tweet_highlights) {
            $('[role=main]').addClass('ntisas-self-tweet-highlights');
        } else {
            $('[role=main]').removeClass('ntisas-self-tweet-highlights');
    ...
    if (Menu.hasSettingChanged('image_popout_enabled') && IsPageType(STREAMING_PAGES)) {
    ...
    if (Menu.hasSettingChanged('display_tweet_statistics') && NTISAS.display_tweet_statistics && !IsPageType(['tweet', 'web_tweet', 'other'])) {
        CollectTweetStats();
    }
    if (Menu.hasSettingChanged('lock_page_enabled')) {
    ...
    if (Menu.hasSettingChanged('media_timeline_enabled') && !NTISAS.media_timeline_enabled) {
        $('.ntisas-media-icon-container').remove();
        $('.ntisas-media-view-icon').remove();
        $('.ntisas-media-tweet').removeClass('ntisas-media-tweet');
    }
    if (Menu.hasSettingChanged('query_subdomain')) {
    ...
    if (Menu.hasSettingChanged('SauceNAO_API_key')) {
        SetSauceAPIKey();
    }
    if (Menu.hasSettingChanged('use_alternate_tweets_API')) {
        Notice.notice("Changing the API endpoint requires a page refresh.", true);
    }
    InitializeSideMenu();
}
...
function SetSauceAPIKey() {
    Saucenao.api_key = NTISAS.SauceNAO_API_key;
}
...
function SetColorValues() {
    let {base_color, text_color, background_color} = NTISAS.colors;
    let background_average = Statistics.average(background_color.map((c) => Number(c)));
    let textinput_color = (background_average > 128 ? background_color : ChangeColorArray(background_color, 25));
    NTISAS.color = {
        base_100: Utility.sprintf('rgb(%s, %s, %s)', ...base_color),
        base_50: Utility.sprintf('rgba(%s, %s, %s, 0.5)', ...base_color),
        base_20: Utility.sprintf('rgba(%s, %s, %s, 0.2)', ...base_color),
        base_dark: Utility.sprintf('rgb(%s, %s, %s)', ...ChangeColorArray(base_color, -50)),
        text_100: Utility.sprintf('rgb(%s, %s, %s)', ...text_color),
        text_50: Utility.sprintf('rgba(%s, %s, %s, 0.5)', ...text_color),
        text_20: Utility.sprintf('rgba(%s, %s, %s, 0.2)', ...text_color),
        text_10: Utility.sprintf('rgba(%s, %s, %s, 0.1)', ...text_color),
        background: Utility.sprintf('rgb(%s, %s, %s)', ...background_color),
        textinput: Utility.sprintf('rgb(%s, %s, %s)', ...textinput_color),
    };
}
...
function RenderSettingsMenu() {
    ...
    $('#ntisas-display-settings').append(Menu.renderCheckbox('display_tweet_views'));
    $('#ntisas-display-settings').append(Menu.renderCheckbox('display_profile_views'));
    $('#ntisas-display-settings').append(Menu.renderCheckbox('self_tweet_highlights'));
    $('#ntisas-display-settings').append(Menu.renderCheckbox('display_user_id'));
    $('#ntisas-display-settings').append(Menu.renderCheckbox('display_image_number'));
    $('#ntisas-display-settings').append(Menu.renderCheckbox('display_tweet_statistics'));
    $('#ntisas-display-settings').append(Menu.renderCheckbox('display_available_sauce'));
    $('#ntisas-display-settings').append(Menu.renderCheckbox('display_network_errors'));
    $('#ntisas-function-settings').append(Menu.renderCheckbox('media_timeline_enabled'));
    $('#ntisas-function-settings').append(Menu.renderCheckbox('advanced_tooltips_enabled'));
    $('#ntisas-function-settings').append(Menu.renderCheckbox('image_popout_enabled'));
    $('#ntisas-function-settings').append(Menu.renderCheckbox('auto_unhide_tweets_enabled'));
    $('#ntisas-function-settings').append(Menu.renderCheckbox('lock_page_enabled'));
    $("#ntisas-query-message").append(Menu.renderExpandable("Additional setting details", QUERY_SETTINGS_DETAILS));
    $('#ntisas-query-settings').append(Menu.renderInputSelectors('IQDB_settings', 'checkbox'));
    $('#ntisas-query-settings').append(Menu.renderInputSelectors('sauce_settings', 'checkbox'));
    $('#ntisas-query-settings').append(Menu.renderTextinput('similarity_cutoff', 10));
    $('#ntisas-query-settings').append(Menu.renderTextinput('results_returned', 10));
    $('#ntisas-query-settings').append(Menu.renderTextinput('SauceNAO_API_key', 80));
    $('#ntisas-network-settings').append(Menu.renderCheckbox('URL_wildcards_enabled'));
    $('#ntisas-network-settings').append(Menu.renderCheckbox('custom_order_enabled'));
    $('#ntisas-network-settings').append(Menu.renderTextinput('recheck_interval', 5));
    $('#ntisas-network-settings').append(Menu.renderInputSelectors('query_subdomain', 'radio'));
    $('#ntisas-network-settings').append(Menu.renderCheckbox('use_alternate_tweets_API'));
    $('#ntisas-download-settings').append(Menu.renderTextinput('filename_prefix_format', 80));
    $('#ntisas-database-controls').append(IMPORT_FILE_INPUT);
    $('#ntisas-database-controls').append(Menu.renderLinkclick('import_data', true));
    $('#ntisas-database-controls').append(Menu.renderInputSelectors('export_types', 'checkbox', true));
    $('#ntisas-database-controls').append(Menu.renderLinkclick('export_data', true));
    $('#ntisas-database-controls').append(IMPORT_ERROR_DISPLAY);
    $('#ntisas-cache-controls').append(Menu.renderLinkclick('cache_info', true));
    $('#ntisas-cache-controls').append(Menu.renderCacheInfoTable());
    $('#ntisas-cache-controls').append(Menu.renderLinkclick('purge_cache', true));
    //Engage jQuery UI
...
async function Main() {
    let guest_token = Utility.readCookie('gt');
    ...
    $(document).on(JSPLib.event.contextmenu, '.ntisas-control-search', SearchControl);
    $(document).on(JSPLib.event.click, '.ntisas-control-confirm', ConfirmControl);
    $(document).on(JSPLib.event.click, '.ntisas-control-upload', UploadMenu);
    $(document).on(JSPLib.event.click, '.ntisas-control-download', DownloadMenu);
    $(document).on(JSPLib.event.contextmenu, '.ntisas-control-download', DownloadControl);
    $(document).on(JSPLib.event.click, '.ntisas-download-media.ntisas-active', DownloadLink);
    $(document).on(JSPLib.event.click, '.ntisas-manual-add', ManualAdd);
    $(document).on(JSPLib.event.click, '.ntisas-help-info', HelpInfo);
    $(document).on(JSPLib.event.click, '.ntisas-image-container a', SelectPreview);
    $(document).on(JSPLib.event.click, '.ntisas-select-controls a', SelectControls);
    $(document).on(JSPLib.event.click, '.ntisas-metric', SelectMetric);
    $(document).on(JSPLib.event.click, '.ntisas-toggle-image-size', ToggleImageSize);
    $(document).on(JSPLib.event.keydown, null, 'alt+q', ToggleSimilarSource);
    $(document).on(JSPLib.event.keydown, null, 'alt+v', ToggleViewHighlights);
    ...
    $(document).on(JSPLib.event.keydown, null, 'alt+1 alt+2 alt+3', SideMenuHotkeys);
...
Load.exportFuncs({debug_list: [GetData, SaveData, MarkupMainTweet, MarkupStreamTweet, BigIntMax, BigIntMin], always_list: [GetImageLinks, GetImageData]});
