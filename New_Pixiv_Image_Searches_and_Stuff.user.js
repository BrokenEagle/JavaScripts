...
// https://www.pixiv.net/en/tags/original
const SEARCH_REGEX = Template.verboseRegex('i')`
${PIXIV_HOST}
/search\?
(?<query>.+)
$
...
const PAGE_ATTRIBUTES = {
    ...
    tag_works: {
        ...
        sections: [{
            selector: 'div#__next > div > div > div > div > div > div > div > div:not(.npisas-artwork) > div > div > a > div > div > img',
            imageparent: 5,
            artworkparent: 6,
            gridparent: 8,
        }],
...
const AI_TAGS = [
    ...
    'AI制作', 'Illustrious', 'IllustriousXL', 'AI着色補助',
];
...
async function CheckIQDB(image_url) {
    ...
    if (filter_results.length) {
        let post_data = Utility.getObjectAttributes(filter_results, 'post');
        ...
        mapped_posts.forEach((post) => {
            ...
            post.uploadername = user?.name ?? 'user #' + user.id;
        });
        SavePosts(mapped_posts);
...
function MenuUploadControl(event) {
    ...
    if (image_count > 1) {
        ...
        if (!NPISAS.confirm_upload || confirm("Upload artwork?")) {
            let link = $menu.find('.npisas-control-upload a').get(0).href;
...
function PreviewUploadControl(event) {
    ...
    let link = $link.find('a').get(0).href;
    GM_openInTab(link, true);
    event.preventDefault();
}
...
function DialogUploadArtwork(event) {
    ...
    let link = $content.find('.npisas-upload-all').get(0).href;
    GM_openInTab(link, true);
}
...
function PageNavigation(pagetype) {
    ...
    if (['user', 'user_works', 'user_tag_works', 'tags', 'tag_works', 'search'].includes(NPISAS.page)) {
...
function ProcessNewArtworks() {
    ...
    for (let i = 0; i < page_attr.sections.length; i++) {
        ...
        if ($artwork_grid.length) {
            ...
            if (['tag_works', 'search'].includes(NPISAS.page)) {
                $artwork_grid.css({
                    'display': 'flex',
            ...
            if (['tags', 'user', 'user_works', 'user_tag_works', 'bookmarks', 'tag_bookmarks', 'artwork'].includes(NPISAS.page)) {
            ...
            if (['tag_works', 'search'].includes(NPISAS.page)) {
                let width_class = [...$artwork_grid.get(0).classList].find((c) => c.startsWith('[width'));
                $('.' + SanitizeCSSName(width_class)).css('width', side_padding + 'px');
                let $nav_ul = $('nav[data-ga4-label="tags_carousel"] ul');
                $nav_ul.prev().css('width', '6em');
                $nav_ul.parent().css('-webkit-mask-image', 'none');
                $nav_ul.children().css('padding', '0px');
                Utility.recheckInterval({
                    check: () => $nav_ul.children().css('padding') !== '0px',
                    success: () => $nav_ul.children().css('padding', '0px'),
                    interval: 500,
