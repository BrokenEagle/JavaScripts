...
const relatedtag_constraints = {
    ...
    value: {
        ...
        tags_overlap: JSPLib.validate.hash_constraints, //Need to fix this before releasing major version
...
function InitializeRelatedTagColumnWidths() {
    ...
    $('.tag-column', $related_tags[0]).each((i,column)=>{
        let $column = $(column);
        $column.css('width', "");
        let max_width = Math.max(...$('a', column).map((i,entry)=>{return $(entry).width();}));
        if ($column.hasClass('general-related-tags-column')) {
            max_width += 38; //2.7em
        } else if ($column.hasClass('artist-related-tags-column')) {
            max_width += 14; //1em
        }
    ...
    if ($related_tags.prop('scrollWidth') > ($related_tags.outerWidth() + 28)) {
        ...
        $("#iac-edit-scroll-bar").width($related_tags.prop('scrollWidth'));
