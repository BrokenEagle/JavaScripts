...
const SETTINGS_CONFIG = {
    ...
    source_order: {
        ...
        default: tag_sources,
        validate: (data) => (Array.isArray(data) && JSPLib.utility.arrayEquals(data, tag_sources)),
...
function MetatagData() {
    if (!MetatagData.data) {
        MetatagData.data = Danbooru.Autocomplete.METATAGS
            .filter((tag) => (tag[0] !== '-'))
    ...
    if ((IAC.controller === 'posts' && IAC.action === 'show') || (IAC.controller === 'uploads' && IAC.action === 'new')) {
        ...
        if (IAC.controller === 'posts') {
            ...
            if (/^\s+$/.test($('.source-related-tags-columns').html())) {
                ...
                JSPLib.utility.setupMutationReplaceObserver('.related-tags', '.source-related-tags-columns', SaveArtistData);
            } else {
                SaveArtistData();
            }
            InitializeShowRelatedTags();
...
function BroadcastIAC(ev) {
    this.debug('log',`(${ev.data.type}): ${ev.data.name} ${ev.data.key}`);
    switch (ev.data.type) {
...
//Export JSPLib
JSPLib.load.exportData(PROGRAM_NAME, IAC, {}, ['cached_data']);
JSPLib.load.exportFuncs(PROGRAM_NAME, [], [FindArtistSession, InitializeAutocompleteIndexed]);
